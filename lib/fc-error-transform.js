...
class DynamicLinkLibraryMissingProcessor extends ErrorProcessor {
  prefix = "error while loading shared libraries: ";
  suffix = ": cannot open shared object file: No such file or directory";
  debianPakcageUrlPrefix = "https://packages.debian.org/search?lang=en&suite=jessie&arch=amd64&mode=path&searchon=contents&keywords=";
  libPrefixWhiteList = ['/usr/lib/x86_64-linux-gnu', '/lib/x86_64-linux-gnu', '/usr/local/lib'];
  ...
  async process(message) {
    ...
    if (packageName) {
      process.nextTick(() => {
        console.log(red(`Tips: Fun has detected that you are missing ${lib} library, you can try to install it in the following way:
...
Please reinstall it in the following ways:
