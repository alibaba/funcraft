...
const { readLines } = require('../fc-utils/fc-fun-nas-server/lib/file');
...
async function resolveLibPathsFromLdConf(baseDir, codeUri) {
  ...
  const confdPath = path.isAbsolute(codeUri) ? codeUri : path.join(baseDir, codeUri, '.fun/root/etc/ld.so.conf.d');
