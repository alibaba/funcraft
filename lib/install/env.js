...
function addEnv(envVars, nasConfig) {
  ...
  envs['PYTHONUSERBASE'] = generatePathToPython(envs);
...
function generatePathToPython(envs) {
  const defaultPythonPath = '/code/.fun/python';

  let pythonPath;

  if (envs['PYTHONUSERBASE']) {
    pythonPath = `${defaultPythonPath}:${envs['PYTHONUSERBASE']}`;
  } else {
    pythonPath = defaultPythonPath;
  }
  return duplicateRemoval(pythonPath);
}
...
function generateDefaultLibPath(prefix) {
  return [`${prefix}/usr/lib`, `${prefix}/usr/lib/x86_64-linux-gnu`, `${prefix}/lib/x86_64-linux-gnu`].join(':');
}
