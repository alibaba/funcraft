...
class AptTask extends InstallTask {
  ...
  async instDeb() {
    ...
    await this._exec(['bash', '-c', '-O', 'nullglob', `for f in ${this.cacheDir}/archives/*.deb; do dpkg -x $f ${instDir}; mkdir -p ${this.cacheDir}/deb-control/\${f%.*}; dpkg -e $f ${this.cacheDir}/deb-control/\${f%.*}; if [ -f "${this.cacheDir}/deb-control/\${f%.*}/postinst" ]; FUN_INSTALL_LOCAL=true ${this.cacheDir}/deb-control/\${f%.*}/postinst configure; fi; done;`]);
  }
