...
class Task {
  constructor(name, runtime, codeUri, context, verbose = false) {
    this.name = name;
...
class InstallTask extends Task {
  ...
  constructor(name, runtime, codeUri, pkgName, local, context, verbose) {
    super(name, runtime, codeUri, context, verbose);
  ...
  async doRun() {
    ...
    if (this.local){
      ...
      await this.runner.exec(['pip', 'install', '--user', '--no-warn-script-location', this.pkgName], 
        [`PYTHONUSERBASE=/code/.fun/python`, 'PIP_DISABLE_PIP_VERSION_CHECK=1'], this.verbose);
    } else {
...
class ShellTask extends Task {
  constructor(name, runtime, codeUri, script, context, verbose){
    super(name, runtime, codeUri, context, verbose);
    this.script = script;
  }
