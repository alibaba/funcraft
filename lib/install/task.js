...
class Task {
  ...
  async beforeRun() {
    if (!this.context.runner) {
      this.runner = await startInstallationContainer({ runtime: this.runtime, codeUri: this.codeUri, targets: this.context.targets });
    } else {
...
class PipTask extends InstallTask {
  async doRun() {
    ...
    if (this.local || this.target) {
      let pythonUserBase = path.join(this.target || '/code/.fun', 'python');
...
class AptTask extends InstallTask {
  ...
  async instDeb() {
    ...
    await this._exec(['bash', '-c', '-O', 'nullglob', `for f in ${this.cacheDir}/archives/*.deb; do echo "$f"; dpkg -x $f ${instDir}; mkdir -p ${this.cacheDir}/deb-control/\${f%.*}; echo "aaaa"; dpkg -e $f ${this.cacheDir}/deb-control/\${f%.*}; echo "bbb"; if [ -f "${this.cacheDir}/deb-control/\${f%.*}/postinst" ]; then echo "ccc"; FUN_INSTALL_LOCAL=true ${this.cacheDir}/deb-control/\${f%.*}/postinst configure; fi; done;`]);
  }
