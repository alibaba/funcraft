...
async function remoteBuild(data) {
  ...
  if (!body.ok) {
    let err = new Error(`Builf failed, ${body.message}`);
    err.name = 'BuildError';
    err.data = data;
    return;
  }
...
async function buildDeps(func, rootDir, type = 'remote') {
  ...
  if (!hasPackageFile) {
    debug('The package.json inexists in Project.');
    debug('Skipped');
    return;
  }
  ...
  if (Object.keys(dependencies).length === 0) {
    debug('The package.json has not any dependencies in Project.');
    debug('Skipped');
    return;
  }
  ...
  if (hasZip && hasSign) {
    ...
    if (sign === kitx.md5(zip, 'hex')) {
      console.log('The node_modules pre-compressed.');
      console.log('Skip');
      return zip;
    }
  }
