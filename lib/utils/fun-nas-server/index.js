...
// get userid and groupid of specific path
app.get('/path/stat', async(req, res) => {
  console.log('received path/stat request, query is: ' + req.query);
  const nasPath = req.query.remoteNasPath;

  const stat = await fs.lstat(nasPath);
  res.send(stat);
});
