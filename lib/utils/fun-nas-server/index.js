...
app.post('/file/chunk/read', async(req, res) => {
  const path = req.body.path;
  const fileSize = req.body.fileSize;
  const fileStart = parseInt(req.body.fileStart, 10);

  const chunkFileBuf = await readFileChunk(path, fileStart, fileSize);

  res.send(chunkFileBuf);
});
