...
async function convertMountPointToLocal(baseDir, mountPoint) {
...
async function convertMountPointsToNasMappings(baseDir, mountPoints) {
  ...
  for (let mountPoint of mountPoints) {
    const nasMapping = await convertMountPointToLocal(baseDir, mountPoint);
...
async function convertNasConfigToNasMappings(baseDir, nasConfig, serviceName) {
  ...
  if (isNasAuto) { // support 'NasConfig: Auto'
    ...
    if (!(await fs.pathExists(localNasDir))) {
      await path.ensureDir(localNasDir);
    }
