...
const { handleRamException } = require('./error-message');
...
async function findNasFileSystem(nasClient, region, description) {
  ...
  do {
    ...
    try {
      ...
      handleRamException(ex, 'AliyunNASFullAccess');
    }
...
async function convertMountPointToNasMapping(baseDir, mountPoint) {
  const { mountSource, mountDir, serverPath } = resolveMountPoint(mountPoint);

  const nasDir = path.join(baseDir, '.fun', 'nas', serverPath);
...
async function convertMountPointsToNasMappings(baseDir, mountPoints) {
  if (!mountPoints) { return []; }

  const nasMappings = [];

  for (let mountPoint of mountPoints) {
    const nasMapping = await convertMountPointToNasMapping(baseDir, mountPoint);
...
async function convertNasConfigToNasMappings(baseDir, nasConfig, serviceName) {
  if (!nasConfig) { return []; }
  ...
  return await convertMountPointsToNasMappings(baseDir, mountPoints);
  
}

async function convertTplToServiceNasMappings(baseDir, tpl) {
  ...
  for (const { serviceName, serviceRes } of definition.findServices(tpl.Resources)) {    
    ...
    const nasMappings = await convertNasConfigToNasMappings(baseDir, nasConfig, serviceName);
