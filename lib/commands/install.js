...
const { supportedRuntimes } = require('../common/model/runtime');
...
async function installAll(funcPath, { verbose }) {
  ...
  if (!funcPath) {
    ...
    const dir = await getCodeUri(funcPath);
...
async function getCodeUri(funcPath) {
  ...
  if(funcPath){
    ...
    if(functionRes.Properties && functionRes.Properties.CodeUri){
      return functionRes.Properties.CodeUri;
    }

    throw new Error(`Error: can not find CodeUri in function ${funcPath}`);
...
function getRuntime(codeUri, options) {
  let moduleRuntime;
...
async function install(packages, options) {
  ...
  const codeUri = await getCodeUri(options.function);

  const runtime = getRuntime(codeUri, options);
