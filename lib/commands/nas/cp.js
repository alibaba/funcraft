...
async function cp(context) {
  ...
  const srcIsNasProtocol = isNasProtocol(srcPath);
  const dstIsNasProtocol = isNasProtocol(dstPath);
  if (srcIsNasProtocol && !dstIsNasProtocol) {
    //nas => local
    console.log('  Not support NAS file download now!');
    return;
  } else if (!srcIsNasProtocol && dstIsNasProtocol) {
    //local => nas
    srcPath = resolveLocalPath(srcPath);
    try {
    ...
    upload(srcPath, resolvedDst, commonPath, isRecursive);
