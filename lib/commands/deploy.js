...
async function makeFunction(fc, serviceName, func) {
  ...
  const calcEnv = {}
  for (let key in func.env) {
    let value = func.env[key];
    let matcher = null == value ? null : value.match(/^\${(\S+)}$/)
    calcEnv[key] = func.env[key]
    if (null == value || null != matcher) {
      let realKey = (null != matcher && null == matcher[1]) ? matcher[1] : key;
      let envValue = process.env[realKey]
      if ("undefined" !== typeof (envValue) && null != envValue) {
        calcEnv[key] = process.env[realKey]
      }
    }
  }
