...
const { detectTplPath, getTpl, validateYmlName, getProjectRoot } = require('../../tpl');
const { getEvent } = require('../../utils/file');
const { red, yellow } = require('colors');
const { DEFAULT_NAS_PATH_SUFFIX } = require('../../tpl');

async function localInvoke(invokeName, tpl, debugPort, event, debugIde, baseDir, tmpDir, debuggerPath, debugArgs, reuse = true, nasBaseDir) {
...
async function invoke(invokeName, options) {
  ...
  const projectRoot = getProjectRoot(tplPath);
  const nasBaseDir = path.join(projectRoot, DEFAULT_NAS_PATH_SUFFIX);

  await localInvoke(invokeName, tpl, debugPort, event, debugIde, baseDir, options.tmpDir, debuggerPath, debugArgs, reuse, nasBaseDir);
}
