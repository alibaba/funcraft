...
async function localInvoke(invokeName, tpl, debugPort, event, debugIde, baseDir, tmpDir) {
  ...
  const absTmpDir = tmpDir ? path.resolve(tmpDir) : path.resolve(baseDir, codeUri, '.fun', 'tmp', serviceName, functionName);

  await makeTmpDir(absTmpDir);
...
async function makeTmpDir(absTmpDir) {
