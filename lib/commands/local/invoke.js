...
async function localInvoke(invokeName, tpl, debugPort, event, debugIde, baseDir) {
  ...
  const localInvoke = new LocalInvoke(serviceName, serviceRes, functionName, functionRes, debugPort, debugIde, baseDir);
...
async function invoke(invokeName, options) {
  ...
  if (!tplPath) {
    ...
    await localInvoke(invokeName, tpl, debugPort, event, debugIde, baseDir);
  } else {
