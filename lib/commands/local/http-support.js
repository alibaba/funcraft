...
async function registerSingleHttpTrigger(app, router, serverPort, httpTrigger, debugPort, debugIde, baseDir, eager = false, debuggerPath, debugArgs, nasBaseDir) {
  ...
  const endpointPrefix = path ? path : `/2016-08-15/proxy/${serviceName}/${functionName}`;
  const endpoint = path ? endpointPrefix : `${endpointPrefix}*`;
  ...
  printHttpTriggerTips(serverPort, serviceName, functionName, triggerName, endpointPrefix, httpMethods, authType, domainName);
}
