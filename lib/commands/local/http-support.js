...
async function registerHttpTriggers(app, serverPort, httpTriggers, debugPort, debugIde, baseDir, debuggerPath, debugArgs) {
  for (let httpTrigger of httpTriggers) {
    await registerSingleHttpTrigger(app, serverPort, httpTrigger, debugPort, debugIde, baseDir, debuggerPath, debugArgs);
  }

  console.log();
}

async function registerSingleHttpTrigger(app, serverPort, httpTrigger, debugPort, debugIde, baseDir, eager = false, debuggerPath, debugArgs) {
  ...
  const httpInvoke = new HttpInvoke(serviceName, serviceRes, functionName, functionRes, debugPort, debugIde, baseDir, authType, endpointPrefix, debuggerPath, debugArgs);
  if (eager) {
...
async function registerApis(app, serverPort, functions, debugPort, debugIde, baseDir, debuggerPath, debugArgs) {
  for (let { serviceName, serviceRes,
    ...
    const apiInvoke = new ApiInvoke(serviceName, serviceRes, functionName, functionRes, debugPort, debugIde, baseDir, debuggerPath, debugArgs);
