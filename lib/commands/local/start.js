...
const { promptForFunctionSelection } = require('../../../lib/init/prompt');
...
async function start(options, invokeName = '') {
  ...
  if (debugPort) {
    ...
    if (httpTriggers.length > 1) {
      const { serviceName, functionName } = await promptForFunctionSelection(httpTriggers);

      debugFunction = httpTriggers.filter(f => {
        return f.serviceName === serviceName && f.functionName === functionName;
      });
