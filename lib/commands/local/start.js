...
function registerApis(app, functions, debugPort, debugIde, tplPath) {
  for (let { serviceName, serviceRes, 
    ...
    app.post(endpoint, async (res, resp) => {
      ...
      await docker.invokeFunction(serviceName, functionName, functionRes, debugPort, event, debugIde, null, outputStream, errorStream, nasConfig, tplPath, true);
...
function registerHttpTriggers(app, httpTriggers, debugPort, debugIde, tplPath) {
  for (let { serviceName, serviceRes,
    ...
    console.log("http endpoint is " + endpointPrefix + "/");
    ...
    for (let method of httpMethods) {
      ...
      app[method.toLowerCase()](endpoint, async (req, res) => {
        ...
        await docker.invokeFunction(serviceName, functionName, functionRes, debugPort, event, debugIde, httpParams, outputStream, errorStream, nasConfig, tplPath, true);

        // todo: 实时处理，而不是最后处理
        const errorResponse = errorStream.toString();
        // todo: response and error response is empty
...
async function start(options) {
  ...
  if (!tplPath) {
    ...
    registerHttpTriggers(app, httpTriggers, debugPort, debugIde, tplPath);

    const functions = definition.findFunctionsInTpl(tpl);

    registerApis(app, functions, debugPort, debugIde, tplPath);
