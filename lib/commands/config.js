...
const { shortDateStr } = require('../profile');
...
async function config() {
  ...
  if (isExists) {
    ...
    if (!shortDateStr(profYml.api_version)) { 
      // 1. The default configuration of fcli is api_version: 2016-08-15
      // 2. js-yaml will parse it according to date when yaml : yaml.JSON_SCHEMA is not configured
      // 3. After js-yaml dump, the generated content is: 2016-08-15T00:00:00.000Z, then it will be written to config.yaml
      // 4. fcli reading to this configuration will cause the request to go wrong
      
      // Here are the following three formats, which are eventually formatted as '2016-08-15':
      // 兼容以下三种格式:
      // 1. Compatible 2016-08-15
      // 2. Compatible with '2016-08-15'
      // 3. Compatible 2016-08-15T00:00:00.000Z
