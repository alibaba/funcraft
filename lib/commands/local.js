...
async function localInvoke(invokeName, tpl, debugPort, event, debugIde) {
  ...
  await docker.invokeFunction(serviceName, functionName, functionDefinition, debugPort, event, debugIde);
}
...
function getDebugIde(options) {
  if (options.config) {
    ...
    console.error(red(`Error parsing debug config. Option is one of: ${JSON.stringify(allowedDebugIdes)}`));
    process.exit(-1);
  }
...
async function local(invokeName, options) {
  ...
  if (!tplPath) {
    ...
    await localInvoke(invokeName, tpl, debugPort, event, debugIde);
  } else {
