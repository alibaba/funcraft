...
async function invokeFunction(serviceName, functionName, functionDefinition, debugPort) {
    // todo: 
    // --name 
    ...
    const handler = [functionProps.Handler];
   ...
   if (debugPort) {
    ...
    const opts = await generateDockerOpts(runtime, mount, debugPort);
...
async function generateDockerOpts(runtime, mount, debugPort) {
    let envs = ['local=true'];
...
async function local(invokeName, options) {
    ...
    if (!tplPath) {
        ...
        await localInvoke(invokeName, tpl, debugPort);
    } else {
...
async function localInvoke(invokeName, tpl, debugPort) {
    ...
    await invokeFunction(serviceName, functionName, functionDefinition, debugPort);
}
