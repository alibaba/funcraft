...
async function describeSecurityGroups(client, region, vpcId, securityGroupName) {
  ...
  console.log("describeRs: " + JSON.stringify(describeRs));
...
async function createSecurityGroup(ecsClient, region, vpcId, securityGroupName) {
  ...
  const createRs = await ecsClient.request('CreateSecurityGroup', params, requestOption);
  console.log("createRs: " + JSON.stringify(createRs));

  const securityGroupId = createRs.SecurityGroupId;

  await authDefaultSecurityGroupRules(ecsClient, region, securityGroupId);
