...
class Invoke {
  ...
  async init() {
    ...
    this.unzipCodeDir = await processZipCodeIfNecessary(this.codeUri);
    this.codeMount = await docker.resolveCodeUriToMount(this.unzipCodeDir || this.codeUri);
  ...
  cleanUnzipCodeDir() {
    if (this.unzipCodeDir) {
      rimraf.sync(this.unzipCodeDir);
      console.log(`clean tmp code dir ${this.unzipCodeDir} successfully`);
      this.unzipCodeDir = null;
    }
  }
