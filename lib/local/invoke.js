...
class Invoke {
  ...
  async init() {
    ...
    this.nasMounts = await docker.resolveNasConfigToMounts(this.serviceName, this.nasConfig, this.baseDir);
    ...
    const resolveImageName = await dockerOpts.resolveRuntimeToDockerImage(this.runtime);

    this.imageName = await docker.pullImageIfNeed(resolveImageName);
  }
