...
class HttpInvoke extends Invoke {
  ...
  async _generateRunner() {
    ...
    const envs = await docker.generateDockerEnvs(this.functionProps, this.debugPort, null, this.nasConfig);
  ...
  async doInvoke(req, res) {
    ...
    await lock.acquire('invoke', async () => {
      ...
      const envs = await docker.generateDockerEnvs(this.functionProps, this.debugPort, httpParams, this.nasConfig);
