...
class HttpInvoke extends Invoke {
  constructor(serviceName, serviceRes, functionName, functionRes, debugPort, debugIde, tplPath, authType, endpointPrefix) {
    ...
    process.on('SIGINT', () => {
      this.cleanUnzipCodeDir();
    });
  }

  _disableRunner(evt, name) {
    let oldRunner = null;
    let dirNeedToClean = this.unzipCodeDir;
    ...
    lock.acquire('invoke', (done) => {
      ...
      oldRunner.stop().catch(reason => {
        ...
        if (dirNeedToClean) {
          rimraf.sync(dirNeedToClean);
          console.log(`clean tmp code dir ${dirNeedToClean} successfully`);
        }
      });
    });
  }
      ...
      if (!this.runner) {
        ...
        await lock.acquire('invoke', async () => {
          ...
          if (!this.runner) {
            ...
            if (!this.watcher) {
              console.log(`watch this.codeUri ${this.codeUri}`);

              this.watcher = watch(this.codeUri, { recursive: true, persistent: false }, (evt, name) => {
                if (this.runner) {
  ...
  async _generateRunner() {
    ...
    const envs = await docker.generateDockerEnvs(this.functionProps, this.debugPort, null, this.nasConfig, true);
  ...
  async doInvoke(req, res) {
    ...
    await lock.acquire('invoke', async () => {
      ...
      const envs = await docker.generateDockerEnvs(this.functionProps, this.debugPort, httpParams, this.nasConfig, true);
