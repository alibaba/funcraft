...
async function matchingFuntionUnderServiceBySourceName(resources, sourceName) {
  ...
  if (functions.length === 0) {
    ...
    const selectionFunction = functions.filter(funcObj => {

      return funcObj.serviceName === _.first(array) && funcObj.funcName === _.last(array);
    });
    
    // delete unmatch functions under a serviceRes
    const serviceRes = deleteUnmatchFunctionsUnderServiceRes(_.first(selectionFunction));

    return {
      serviceName: _.first(selectionFunction).serviceName,
      serviceRes
    };
  }
  return {
    serviceName: _.first(functions).serviceName,
    serviceRes: _.first(functions).serviceRes
  };
}
