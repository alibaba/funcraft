...
const CUSTOM_DOMAIN = 'Aliyun::Serverless::CustomDomain';
...
function findCustomDomain(resources) {
  const customDomain = [];

  iterateResources(resources, CUSTOM_DOMAIN, (domainName, domainDefinition) => {
    customDomain.push({
      domainName,
      domainDefinition
    });
...
function findDomainNameBy(serviceName, functionName, tpl) {

  for (const { domainName, domainDefinition } of findCustomDomain(tpl.Resources)) {
    const properties = (domainDefinition.Properties || {});
    const routeConfig = properties.RouteConfig.Routes || properties.RouteConfig.routes;

    for (const pathConfig of Object.entries(routeConfig)) {
      const routeServiceName = (pathConfig[1] || {}).ServiceName;
      const routeFunctionName = (pathConfig[1] || {}).FunctionName;
      if (serviceName === routeServiceName && functionName === routeFunctionName) {
        return domainName;
      }
    }
  }
...
module.exports = {
  ...
  SERVICE_RESOURCE, FUNCTION_RESOURCE, FLOW_RESOURCE, validateNasAndVpcConfig, findDomainNameBy
};