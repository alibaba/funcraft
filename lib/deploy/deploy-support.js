...
async function makeFunction(baseDir, {
  ...
  if (codeUri && codeUri.startsWith('oss://')) {
    ...
    const {base64, filesCount} = await getFunCodeAsBase64(baseDir, codeUri, runtime);
    console.log(green(`\t\tpackage function ${functionName} code done, the number of files you have packaged is：` + yellow(`${filesCount}`)));
    code = {
      zipFile: base64
    };
  }
