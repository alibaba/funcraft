    ...
    try {
      ...
      if (ex.code === 'AccessDenied') {
        ...
        if (ex.message.indexOf('the caller is not authorized to perform') !== -1) {
          ...
          console.error(red(`You have not given sub-account permission to manage the fc service, The permission link is https://ram.console.aliyun.com/users`));

        } else if (ex.message.indexOf('FC service is not enabled for current user') !== -1) {

          console.error(red(`You have not opened the function computer service, the opening address is https://www.aliyun.com/product/fc`));
