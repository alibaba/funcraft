...
async function getFunCodeAsBase64(baseDir, codeUri, runtime) {
  if (codeUri) {
    if (codeUri.endsWith('.zip') || codeUri.endsWith('.jar')) {
      return {base64: Buffer.from(await readFile(codeUri)).toString('base64')};
    }
  } else {
    codeUri = './';
  }
  ...
  await detectLibrary(codeUri, runtime, baseDir, '\t\t');
...
async function makeFunction(baseDir, {
  ...
  if (codeUri && codeUri.startsWith('oss://')) {
    ...
    const {base64, count} = await getFunCodeAsBase64(baseDir, codeUri, runtime);
    const log = count ? ', the number of files you have packaged is：' + yellow(`${count}`) : '';
    console.log(green(`\t\tpackage function ${functionName} code done` + log));
    code = {
      zipFile: base64
    };
  }
...
async function detectLibrary(codeUri, runtime, baseDir, wrap = '') {
  const stats = await lstat(codeUri);
  if (stats.isFile()) {
    let featureLibrary = runtimeTypeMapping[runtime];

    const allPath = path.join(baseDir, codeUri);
    const dirName = path.dirname(allPath);
    const childDir = await readdir(dirName);

    if (Array.isArray(featureLibrary)) {
      let flag = false;
      for (const iterator of featureLibrary) {
        for (const name of childDir) {
          if (_.endsWith(name, iterator)) {
            console.warn(red(`${wrap}Do you forget to place the ${name}'s directory in the CodeUri corresponding to template.yml.`));
            flag = true;
            break;
          }
        }
      ...
      if (childDir.includes(featureLibrary)) {
        console.warn(red(`${wrap}Do you forget to place the ${featureLibrary}'s directory in the CodeUri corresponding to template.yml.`));
      } else {

        const funDir = childDir.filter(p => p === '.fun');
        if (Array.isArray(funDir) && funDir.length > 0) {

          const childFun = await readdir(path.join(dirName, '.fun'));

          if (childFun.includes('root')) {

            console.warn(red(`${wrap}Do you forget to place the .fun/root's directory in the CodeUri corresponding to template.yml.`));
