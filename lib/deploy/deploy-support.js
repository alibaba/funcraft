...
const fcPartialMethods = [ 'getService', 'updateService', 
  'createService', 'getFunction', 
  'createFunction', 'updateFunction',
  'getTrigger', 'createTrigger',
  'updateTrigger'];
...
const getFcClient = async () => {
  ...
  const fcClient = new FC(profile.accountId, {
  ...
  return Object.getOwnPropertyNames(FC.prototype)
    .filter(p => typeof fcClient[p] === 'function')
    .reduce(function(client, func) {
              
      if( fcPartialMethods.includes(func) ) {
        const oldFunc = client[func];
        client[func] = (...args) => {
          return oldFunc.call(client, ...args, fcHeaders);
        };
      }
      return client;
    }, fcClient);
