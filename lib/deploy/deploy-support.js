...
async function makeApi(group, {
  ...
  await promiseRetry(async (retry, times) => {
    try {
      ...
      if (!_.isEmpty(errorCodeSamples)) {

        params.ErrorCodeSamples = JSON.stringify({
          'Code': errorCodeSamples.code,
          'Message': errorCodeSamples.message,
          'Description': errorCodeSamples.description
        });
      }
