...
async function getFunCodeAsBase64(baseDir, codeUri, runtime) {
  ...
  await featureLibrary(codeUri, runtime, baseDir, '\t\t');
...
const runtimeTypeMapping = {
  ...
  'python2.7': ['.egg-info', '.dist-info'],
  'python3': ['.egg-info', '.dist-info'],
  'php7.2': 'extension'
};

async function featureLibrary(codeUri, runtime, baseDir, wrap = '') {
  const stats = await lstat(codeUri);
  if (stats.isFile()) {
    let featureLibrary = runtimeTypeMapping[runtime];

    const allPath = path.join(baseDir, codeUri);
    const preAllPath = path.resolve(allPath, '..');

    const dirArray = await readdir(preAllPath);
    ...
    } else if (dirArray.includes(featureLibrary)) {
