...
async function makeService({
  ...
  await promiseRetry(async (retry, times) => {
    try {
      ...
      if (ex.code === 'AccessDenied' || ex.code === 'ERR_INVALID_CHAR' || ex.code === 'ENOTFOUND') {
...
async function getSourceArn(triggerType, triggerProperties) {
  ...
  if (triggerType === 'Log') {
    ...
    return `acs:oss:${profile.defaultRegion}:${profile.accountId}:${triggerProperties.BucketName ? triggerProperties.BucketName : triggerProperties.bucketName}`;
...
function getTriggerConfig(triggerType, triggerProperties) {
  if (triggerType === 'Timer') {
    ...
    return {
      events : triggerProperties.Events ? triggerProperties.Events : triggerProperties.events,
      filter : triggerProperties.Filter ? triggerProperties.Filter : triggerProperties.filter
    };
  } else if (triggerType === 'CDN') {
    return {
