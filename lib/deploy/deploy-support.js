...
const getSlsClient = async () => {
  ...
  return new ALY.SLS({
    "accessKeyId": profile.accessKeyId,
    "secretAccessKey": profile.accessKeySecret,
    "endpoint": `https://${profile.defaultRegion}.log.aliyuncs.com`,
...
async function makeLogstore({
  ...
  if (!exists) {
    ...
    try {
      ...
      if (ex.errorCode !== 'ParameterInvalid' && errorMessage !== 'no parameter changed') {
        throw ex;
      }
    }
...
async function makeSlsProject(projectName, description) {
  ...
  if (projectExist) {
    ...
    console.log(`sls project exists, but could not be updated`);
  } else {
    try {
      await slsRequesttoPromise((callback) => {
      ...
      if (ex.errorCode === 'ProjectAlreadyExist') {
        console.error(`error: sls project ${projectName} already exist, it may be in other region.`);
        process.exit(-1);
      } else {
        throw ex;
      }
...
function getProject(sls, projectName) {
  return new Promise(function(resolve, reject) {
    sls.getProject({projectName}, function (err, data) {
      if (!err) {
        resolve(data);
      } else {
        reject(err);
...
async function makeInvocationRole(roleName, description) {
  return await makeRole(roleName, true, description, JSON.stringify({
    'Statement': [
      {
...
async function attachPolicyToRole(policyName, roleName) { 
...
async function makeOtsInstance(instanceName, clusterType, description) {
    ...
    try {
      const response = await pop.request("GetInstance", {
...
async function makeOtsTrigger({
  ...
  const client = await getOtsClient("falskjdfldjsf");
  const response = client.request("CreateTrigger", null, null);
...
async function makeOtsTable({
  ...
  if (!tbExist) {
    ...
    console.log("ots table already exists, but could not be updated");
  }
}
...
module.exports = {
  ...
  makeRole, makeInvocationRole, attachPolicyToRole, makeService, makeTrigger, makeSlsProject, makeLogstore
};