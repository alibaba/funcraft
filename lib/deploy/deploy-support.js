...
const fcHeaders = {
  'user-agent': `Node.js(${process.version}) OS(${process.platform}/${process.arch}) Fun(${pkg.name}@v${pkg.version})`,
};
...
async function makeLogstoreIndex(projectName, logstoreName) {
  ...
  await promiseRetry(async function(retry, times) {
    try {
...
async function makeService({serviceName, 
  ...
  var service;
  try {
    service = await fc.getService(serviceName, fcHeaders);
  } catch (ex) {
  ...
  if (!service) {
    service = await fc.createService(serviceName, options, fcHeaders);
  } else {
    service = await fc.updateService(serviceName, options, fcHeaders);
  }
...
async function makeFunction({
  ...
  var fn;
  try {
    fn = await fc.getFunction(serviceName, functionName, fcHeaders);
  } catch (ex) {
  ...
  if (!fn) {
    ...
    fn = await fc.createFunction(serviceName, params, fcHeaders);
  } else {
    // update
    fn = await fc.updateFunction(serviceName, functionName, params, fcHeaders);
  }
...
async function makeTrigger({
  ...
  var trigger;
  try {
    trigger = await fc.getTrigger(serviceName, functionName, triggerName, fcHeaders);
  } catch (ex) {
  ...
  if (!trigger) {
    ...
    trigger = await fc.createTrigger(serviceName, functionName, params, fcHeaders);
  } else {
    trigger = await fc.updateTrigger(serviceName, functionName, triggerName, params, fcHeaders);
  }
...
module.exports = {
  ...
  makeService, makeTrigger, makeSlsProject, makeLogstore, makeLogstoreIndex
};