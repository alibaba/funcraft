  ...
  await promiseRetry(async (retry, times) => {
    try {
      ...
      if (ex.code === 'AccessDenied' || ex.code === 'ERR_INVALID_CHAR' || ex.code === 'ENOTFOUND') {
        ...
        console.error(red(`The service or function doesn't belong to you\n`+
        `cannot use subuser uid, login name, login name prefix, display name, and others\n`+
        `please enter the correct accountId, which is a purely numeric account ID under your primary account. You can reset your account ID by executing fun config`));
