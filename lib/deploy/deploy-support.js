...
async function getTriggerNameList ({
  ...
  try {
    listTrigger = await fc.listTriggers(serviceName,functionName);
  } catch (error) {
    throw error;
  }

  if (listTrigger) {
    if (listTrigger.data.triggers) {
        triggerNameArray = listTrigger.data.triggers.map(function(nameStr){
        return nameStr.triggerName;
      });
    }
  }
