...
async function makeInvocationRole(serviceName, functionName, triggerType) {
  if (triggerType === 'Log') {
    ...
    const invocationRoleName = ram.normalizeRoleOrPoliceName(`AliyunOSSEventNotificationRole-${serviceName}-${functionName}`);
    ...
    const policyName = ram.normalizeRoleOrPoliceName(`AliyunOssFcvocationPolicy-${serviceName}-${functionName}`);
