...
async function makeService(serviceName, role, description) {
  ...
  if (!service) {
    service = await fc.createService(serviceName, { description, role });
  } else {
    service = await fc.updateService(serviceName, { description, role });
  }
