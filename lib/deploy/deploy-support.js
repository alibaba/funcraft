...
const getFcClient = async () => {
  ...
  var fcClient = new FC(profile.accountId, {
  ...
  if (profile.endpoint) {
    fcClient.endpoint = profile.endpoint;
  }
...
async function getFunCodeAsBase64(baseDir, codeUri) {
  ...
  return await zip.pack(codeUri, funignore(baseDir));
}
...
function getTriggerConfig(triggerType, triggerProperties) {
  ...
  } else if (triggerType === 'MNSTopic' || triggerType === 'TableStore') {
    return {};
  }
...
async function makeTrigger({
  ...
  if (triggerType === 'HTTP') {
    ...
    console.log('    URL: http://%s.%s.fc.aliyuncs.com/2016-08-15/proxy/%s/%s/',
                profile.accountId,
