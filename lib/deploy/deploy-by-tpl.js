...
async function deployFunction(baseDir, serviceName, functionName, functionRes, updateConfig) {
...
async function deployFunctions(baseDir, serviceName, serviceRes, updateConfig) {
  for (const [k, v] of Object.entries(serviceRes)) {
    if ((v || {}).Type === 'Aliyun::Serverless::Function') {

      const beforeDeployLog = updateConfig ? 'config completed' : 'to be deployed';
      const afterDeployLog = updateConfig ? 'config completed' : 'deploy success';

      console.log(`\tWaiting for function ${k} ${beforeDeployLog}...`);
      await deployFunction(baseDir, serviceName, k, v, updateConfig);
...
async function deployService(baseDir, serviceName, serviceRes, updateConfig) {
  ...
  await deployFunctions(baseDir, serviceName, serviceRes, updateConfig);
}
...
async function partialDeployment(context, tpl) {
  ...
  if (nameArray.length > 2) {
    ...
    throw new Error(`Local deployment format error:` + red(` ${resourceName}`) +`, the correct format is ` + green(`serviceName/functionName | serviceName | functionName`));
  }

  if (nameArray.length === 2) {

    const funcRes= definition.findCertainFunctionByServiceAndFunctionName(tpl.Resources, _.first(nameArray), _.last(nameArray));

    serviceName = funcRes.serviceName;
    serviceRes = funcRes.serviceRes;
  } else if (nameArray.length === 1) {

    const matchRes = await definition.matchingFuntionUnderServiceBySourceName(_.first(nameArray), tpl.Resources);
...
async function deploy(tplPath, context) {
  ...
  for (const [name, resource] of Object.entries(tpl.Resources)) {
    if (resource.Type === 'Aliyun::Serverless::Service') {
      ...
      const beforeDeployLog = context.updateConfig ? 'config completed' : 'to be deployed';
      const afterDeployLog = context.updateConfig ? 'config completed' : 'deploy success';

      console.log(`Waiting for service ${name} ${beforeDeployLog}...`);
      await deployService(baseDir, name, resource, context.updateConfig);
...
module.exports = { deploy, deployCustomDomain, partialDeployment};