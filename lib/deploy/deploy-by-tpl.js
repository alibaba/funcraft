...
async function deployByApi(baseDir, tpl, context) {
...
async function deploy(tplPath, context) {
  ...
  if (context.useRos) {
    if (context.stackName) {
      console.log(yellow(`deploy using ros, stack name is ${context.stackName}`));
    } else {
    ...
    await deployByApi(baseDir, tpl, context);
  }
}
