...
async function deployCustomDomain(domainName, domainDefinition) {
  ...
  for (var route of transRoute) {
    let deformedRoute = _.mapKeys(route[1], function(value, key) {
  ...
  if (Reflect.ownKeys(certConfig).length === 0 && protocol === 'HTTP,HTTPS') {
    ...
    throw new Error(red('Missing of certificate information configuration in template.yml.'));
  }

  if (Reflect.ownKeys(certConfig).length > 0 && protocol === 'HTTP') {

    throw new Error(red('Please delete the configuration of certificate information or replace the protocol with HTTP,HTTPS.'));
  }
