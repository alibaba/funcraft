...
async function processTemporaryDomain(domainLogicId, resources, { serviceName, functionName }) {
  ...
  if (expiredTime > currentTimestamp) {
    ...
    console.log(yellow(`The assigned temporary domain is ${domainName}，expired at ${date.format(expiredTimeObj, 'YYYY-MM-DD HH:mm:ss')}, limited by ${timesLimit} per day.`));
  } else {
    console.log(yellow(`The temporary domain ${domainName} of previous depoyment is expried.`));
  }
...
async function processTemporaryDomainIfNecessary(domainLogicId, domainDefinition, resources) {
  ...
  const domainName = await processTemporaryDomain(domainLogicId, resources, _.head(_.values(routes)));
