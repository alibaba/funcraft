...
async function deployFcService(serviceName, serviceDefinition) {
  ...
  let logConfig = ((serviceDefinition.Properties || {})).LogConfig || {};
  let roleName;
  ...
  await deployLogs(serviceDefinition);
...
async function deployLogs(serviceDefinition) {
  for (const [projectName, v] of Object.entries(serviceDefinition)) {
