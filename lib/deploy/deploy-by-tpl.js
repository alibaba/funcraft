...
async function deployFunction(baseDir, serviceName, functionName, functionRes, onlyConfig) {
...
async function deployFunctions(baseDir, serviceName, serviceRes, onlyConfig) {
  for (const [k, v] of Object.entries(serviceRes)) {
    if ((v || {}).Type === 'Aliyun::Serverless::Function') {
      ...
      const beforeDeployLog = onlyConfig ? 'config completed' : 'to be deployed';
      const afterDeployLog = onlyConfig ? 'config completed' : 'deploy success';

      console.log(`\tWaiting for function ${k} ${beforeDeployLog}...`);
      await deployFunction(baseDir, serviceName, k, v, onlyConfig);
...
async function deploy(tplPath, context) {
  ...
  for (const [name, resource] of Object.entries(tpl.Resources)) {
    if (resource.Type === 'Aliyun::Serverless::Service') {
      ...
      const beforeDeployLog = context.onlyConfig ? 'config completed' : 'to be deployed';
      const afterDeployLog = context.onlyConfig ? 'config completed' : 'deploy success';
