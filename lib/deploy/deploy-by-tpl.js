...
async function deployFlow(name, resource, tpl, parameterOverride = {}, baseDir) {
  ...
  if (properties.Definition) {
    ...
    const definitionObj = parseYamlWithCustomTag(properties.DefinitionUri, await readFile(definitionUri, 'utf8'));
...
async function fetchRemoteYml(baseDir, tpl) {
  const importTmpDir = path.join(baseDir, '.fun', 'tmp');
  ...
  console.log("Collecting your services infomation, in order to caculate devlopment changes...")
  
  for (const service of services) {
    const originConsoleLog = console.log;
    console.log = debug;
    try {
      await importService(service.serviceName, importTmpDir, true, true, true, false);
    } catch(e) {
...
async function deployByApi(baseDir, tpl, tplPath, context) {
  ...
  if (resourceName) {
    ...
    if (resourceType === definition.SERVICE_RESOURCE) {
      ...
      if (!context.assumeYes && !await promptForConfirmContinue('Please confirm to continue.')) {
        return;
      }
