...
const { getTpl, getNasYmlPath } = require('../tpl');
...
const { transformFunctionInDefinition } = require('../fnf');
...
async function deployFlow(name, resource, tpl, parameterOverride = {}) {
  const properties = (resource.Properties || {});
  const description = properties.Description || '';
  const definitionObj = parseYamlWithCustomTag(properties.DefinitionUri, await readFile(properties.DefinitionUri, 'utf8'));
  const definition = transformFunctionInDefinition(definitionObj, tpl, parameterOverride);
  const roleArn = properties.Role;
  const policies = properties.Policies;
...
async function deployByApi(baseDir, tpl, tplPath, context) {
  ...
  if (serviceName) {
    await deployTplService(baseDir, serviceName, serviceRes, context.onlyConfig, tplPath);
    return;
  }
  ...
  for (const [name, resource] of Object.entries(tpl.Resources)) {
    if (resource.Type === 'Aliyun::Serverless::Service') {
      ...
      await deployFlow(name, resource, tpl, context.parameterOverride);
    } else {
...
function showTipsForNasYml(serviceNasMappings) {
  ...
  _.forEach(serviceNasMappings, (nasMappings, key) => {
    for (const nasMapping of nasMappings) {
      localNasDir.push(path.resolve(nasMapping.localNasDir));
    }
  });
  console.log(yellow(`
