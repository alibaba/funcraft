...
function contain(arr1,arr2){  
  if (!arr2.every(x => _.includes(arr1,x))) {  
    return false;  
  }  
  return true;  
}
...
async function deployTriggers(serviceName, functionName, events) {
  if (events) {
    ...
    let localTriggerName = Object.keys(events);
    let onLineTriggerName = await getTriggerNameList({serviceName,functionName});

    if (!contain(localTriggerName, onLineTriggerName)) {
      console.warn(red(`\t\twarning: Trigger is configured online, but there is no local. Please ensure that the online configuration of the triggers is removed before deploying`));
    }
