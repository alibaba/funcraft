...
async function generateServiceRole({ serviceName, defaultRegion, roleArn, policies,
  vpcConfig,
  nasConfig,
  logConfig
}) {
  ...
  if (roleArn === undefined || roleArn === null) {
    roleName = `aliyunfcgeneratedrole-${defaultRegion}-${serviceName}`;
...
async function deployFunction(baseDir, serviceName, functionName, functionRes, nasConfig, vpcConfig, onlyConfig, tplPath) {
...
async function deployFunctions(baseDir, serviceName, serviceRes, onlyConfig, tplPath) {
  ...
  for (const [k, v] of Object.entries(serviceRes)) {
    if ((v || {}).Type === 'Aliyun::Serverless::Function') {
      ...
      await deployFunction(baseDir, serviceName, k, v, serviceProps.NasConfig, serviceProps.VpcConfig, onlyConfig, tplPath);
...
async function deployService(baseDir, serviceName, serviceRes, onlyConfig, tplPath) {
  ...
  const { role, roleName } = await generateServiceRole({ serviceName, roleArn, policies,
  ...
  await deployFunctions(baseDir, serviceName, serviceRes, onlyConfig, tplPath);
}
...
async function deployByApi(baseDir, tpl, tplPath, context) {
  ...
  for (const [name, resource] of Object.entries(tpl.Resources)) {
    if (resource.Type === 'Aliyun::Serverless::Service') {
      ...
      await deployService(baseDir, name, resource, context.onlyConfig, tplPath);
...
module.exports = {
  ...
  generateServiceRole, deployFunctions
};