...
function parseParameters(parametersInTpl, parameterOverride, originParameters) {
...
async function deployByRos(baseDir, stackName, tpl, assumeYes, parameterOverride = {}) {
  ...
  let changeSetId;
  if (!stackId) { // create
    const changeSet = await createChangeSet(rosClient, region, stackName, tpl, parseParameters(tpl.Parameters, parameterOverride));
    ...
    changeSetId = await updateChangeSet(
      rosClient, region, stackName, stackId, tpl, parseParameters(tpl.Parameters, parameterOverride, res.Parameters));
  }
