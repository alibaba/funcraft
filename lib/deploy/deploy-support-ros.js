...
async function waitComplete(rosClient, stackId, region) {
  ...
  do {
    ...
    if (res.Status.indexOf('COMPLETE') !== -1) {

      status = res.Status;
    } else {

      console.log(`\t\twaiting for status to be 'COMPLETE', now is ${res.Status}`);
    }
    
  } while (count < 10 && !status);

  if (!status || status.indexOf('COMPLETE') === -1) { throw new Error(`Timeout while waiting for status to be 'COMPLETE'`); }
}
