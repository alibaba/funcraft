...
async function waitComplete(rosClient, stackId, region) {
  ...
  do {
    ...
    if ((res.Status.indexOf(COMPLETE) !== -1) || (res.Status.indexOf(FAILED) !== -1)) {
  ...
  if (status.indexOf(FAILED) !== -1) { throw new Error(`ROS deployment failed, the reason is: ${status.StatusReason}`); }
}
