...
const outputsTableHeads = ['OutputId', 'Value', 'Description'];
...
async function getStackOutputs(rosClient, stackId, region) {
  ...
  debug('get stacks, params %s', params);
...
async function deployByRos(baseDir, stackName, tpl, assumeYes) {
  ...
  const outputs = await getStackOutputs(rosClient, stackId, region);

  await displayOutputs(outputs);
