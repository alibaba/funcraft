...
const { parseParameters} = require('../build/parser');
...
const outputsTableHeads = ['OutputId', 'Condition', 'Value', 'Description'];
...
async function displayChanges(changes) {
  if (_.isEmpty(changes)) { return; }
  console.log(`\nResource stack changes list:\n`);
...
async function displayOutputs(outputs) {
  ...
  console.log('Resource stack output list:\n');

  const table = getTableInstance(outputsTableHeads);

  for (const [outputKey, outputValue] of Object.entries(outputs)) {

    table.push([outputKey, outputValue.Condition, JSON.stringify(outputValue.Value), outputValue.Description]);
  }
...
function getOutputsFrom(templateBody) {
  ...
  try {
    ...
    console.error(red(`Unable to parse JSON Object. Error: ${err}`));
  }
  return template.Outputs;
}

async function displayParameters(parameters) {
  if (_.isEmpty(parameters)) { return; }

  console.log('Resource stack parameters list:\n');
...
async function localDiffDisplay(changes, templateBody, parameters) {
  await displayChanges(changes);
  await displayOutputs(getOutputsFrom(templateBody));
  await displayParameters(parameters);
}

async function deployByRos(baseDir, stackName, tpl, assumeYes, parameterOverride) {

  const parsedParameters = parseParameters(tpl.Parameters, parameterOverride);
  ...
  if (!stackId) { // create
    ...
    const changeSet = await createChangeSet(rosClient, region, stackName, tpl, parsedParameters);

    changeSetId = changeSet.changeSetId;
    stackId = changeSet.stackId;

  } else { // update
    changeSetId = await updateChangeSet(rosClient, region, stackName, stackId, tpl, parsedParameters);
  }

  const { changes, templateBody, parameters } = await getChangeSet(rosClient, changeSetId, region);
  // changeSet table
  await localDiffDisplay(changes, templateBody, parameters);
