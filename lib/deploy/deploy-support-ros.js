...
const tableHeads = ['Id', 'ResourceType', 'Action', 'Property'];

const eventTableHeads = ['Id', 'Status'];
...
async function findRosStack(rosClient, region, stackName) {
  ...
  do {
    ...
    console.log('param:' + JSON.stringify(params,null,4));
    console.log('requestOption:' + JSON.stringify(requestOption,null,4));

    const rs = await rosClient.request('ListStacks', params, requestOption);
    console.log('rs:' + JSON.stringify(rs,null,4));
...
async function deployByRos(baseDir, stackName, tpl, assumeYes) {
  ...
  await saveTemplate(baseDir, rosTemplateData);
...
async function saveTemplate(baseDir, rosTemplateData) {
  ...
  await fs.outputFile(rosTemplatePath, rosTemplateData);
}
