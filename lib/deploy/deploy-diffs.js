...
const DEFAULT_VALUES = {
  "Aliyun::Serverless::Service": {
    ...
    Role: ({ action, oldPropsValue }) => {
      if (action === 'Delete') {
        return oldPropsValue && oldPropsValue.startsWith("acs:ram::") && _.includes(oldPropsValue, "role/aliyunfcgeneratedrole-cn-");
      } else return false;
    },
    ...
    NasConfig: ({ action, newPropsValue }) => {
      if (action === 'Modify') {
        // todo: 只有是 NasConfig: Auto，且远端是 auto 创建的资源，才能认为无变化，这里先忽略这个规则，只要是 Auto，且远端配置了 NasConfig，就认为没有变化
  ...
  "Aliyun::Serverless::Function": {
    ...
    EnvironmentVariables: ({ resourceType, action, oldPropsValue, newPropsValue, localProps }) => {
      if (action === 'Delete') {
...
function processServiceResources(resources, localResources, remoteResources) {
  const changes = [];
  ...
  for (const [key, res] of Object.entries(resources)) {
    ...
    if (props) {
      ...
      const remoteProps = (remoteRes || {}).Properties;
      ...
      for (const [propsKey, propsValue] of Object.entries(props)) {
        ...
        if (isDefaultValue(resourceType, action, originPropsKey, propsValue, localProps)) {
          continue;
        }

        // 内嵌的对象属性更新了，比如 VpcConfig 里面的值变化了
        if (action === null) action = 'Modify';
        if (originPropsKey === 'CodeUri') action = 'Modify';
    ...
    // 递归处理其他 resources
    changes.push(...processServiceResources(res, localRes, remoteRes));
  }
...
async function showResourcesChanges(localYml, remoteYml) {
  ...
  changes.push(...processServiceResources(resources, localResources, remoteResources));
