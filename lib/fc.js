...
async function processJavaRuntimeIfNecessary({ runtime, baseDir, tplPath, tpl }) {
...
async function processNasAutoConfiguration({ tpl, tplPath, runtime, codeUri, convertedNasConfig, stage,
  ...
  let updatedTplContent = await processCustomRuntimeSpringBootIfNecessary({
    runtime, baseDir,
    codeUri,
    tpl
  });
  ...
  updatedTplContent = await processJavaRuntimeIfNecessary({ runtime, baseDir, tplPath, tpl: rs.updatedTpl });
