...
async function processNasAutoConfiguration({ tpl, tplPath, runtime, codeUri, convertedNasConfig, stages,
  serviceName,
  functionName
}) {
  ...
  if (_.includes(stages, 'package')) { return rs.tplChanged; }
...
async function nasAutoConfigurationIfNecessary({ stages, tplPath, runtime, codeUri, nasConfig, vpcConfig,
  compressedSize,
  nasFunctionName,
  nasServiceName
}) {
  ...
  if (compressedSize > 52428800 && _.includes(SUPPORT_RUNTIMES, runtime)) { // 50M
    ...
    const packageStage = _.includes(stages, 'package');
    ...
    if (await promptForConfirmContinue(`Do you want to let fun to help you automate the configuration?`)) {
      ...
      if (definition.isNasAutoConfig(nasConfig)) {
        ...
        if (yes) {
          ...
          tplChanged = await processNasAutoConfiguration({ tpl, tplPath, runtime, codeUri, stages,
        ...
        if (yes) {
          ...
          tplChanged = await processNasAutoConfiguration({ tpl: updatedTpl, tplPath, runtime, codeUri, stages,
            serviceName: nasServiceName,
            functionName: nasFunctionName
          });
        } else {
