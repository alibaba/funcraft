...
async function processNasAutoConFigurationForNasAuto({ tpl, tplPath, runtime, codeUri,
  nasServiceName,
  nasFunctionName
}) {
  ...
  const { envs, nasMappings } = await generateNasMappingsAndEnvs({
    ...
    runtime,
    codeUri,
    nasConfig: (serviceRes.Properties || {}).NasConfig
  });
  ...
  await saveNasMappings(getNasYmlPath(tplPath), nasMappings);
...
async function autoConfigurationForNasAuto({ serviceName, functionName,
  tplPath,
  nasConfig,
  runtime,
  codeUri,
  ...
  if (compressedSize > 52428800 && _.includes(SUPPORT_RUNTIMES, runtime)) {
    ...
    if (definition.isNasAutoConfig(nasConfig)) {
      ...
      if (yes) {
        ...
        await processNasAutoConFigurationForNasAuto({ tpl, tplPath, runtime, codeUri,
      ...
      if (yes) {
        ...
        await processNasAutoConFigurationForNasAuto({ tpl: updatedTpl, tplPath, runtime, codeUri, nasConfig,
          nasServiceName: serviceName,
          nasFunctionName: functionName
        });
