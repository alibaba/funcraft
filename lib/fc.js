...
async function makeFunction(baseDir, {
  ...
  try {
    ...
    if (ex.message.indexOf('timeout') !== -1) {
      ...
      throw new Error('deploy failed: we speculate that it may be due to network reasons or that the code uploaded is too large. It is recommended that you adopt the following ways:\n1.use "fun config" to increase timeout.\n2.use .funignore or webpack to tailor the package size.');
    }
    throw ex;
  }
