...
const SUPPORT_RUNTIMES = ['nodejs6', 'nodejs8', 'nodejs10', 'python2.7', 'python3', 'custom'];
...
async function nasAutomationConfigurationIfNecessary({ tplPath, runtime, baseDir, codeUri, nasConfig, vpcConfig,
  ...
  if (compressedSize > 12345 && _.includes(SUPPORT_RUNTIMES, runtime)) { // 50M
      ...
      if (definition.isNasAutoConfig(nasConfig)) {
        ...
        if (yes) {
          ...
          await processNasAutomationConfiguration({
            tpl, tplPath, baseDir, runtime, codeUri,
            nasServiceName,
            nasFunctionName
          });
        ...
        if (onlyOneNas) {
          ...
          await backupTemplateFile(tplPath);
          await processNasAutomationConfiguration({
            tpl, tplPath, baseDir, runtime, codeUri, convertedNasConfig,
            nasServiceName,
            nasFunctionName
          });
        }
        ...
        if (yes) {
          ...
          await processNasAutomationConfiguration({
            tpl: updatedTpl, tplPath, baseDir, runtime, codeUri,
            nasServiceName,
            nasFunctionName
          });
        } else {
  ...
  if (!onlyConfig) { // ignore code
    ...
    if (codeUri && codeUri.startsWith('oss://')) { // oss://my-bucket/function.zip
      ...
      const stop = await nasAutomationConfigurationIfNecessary({
        compressedSize, tplPath, baseDir: getBaseDir(tplPath), runtime, nasConfig, vpcConfig,
