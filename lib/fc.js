...
const artifact = require('./build/artifact');
...
function extractPrefix(handler) {
  const splitArray = handler.split('::handleRequest');
...
async function processJavaIfNecessary({
  ...
  const builtTplPath = path.join(rootArtifactsDir, 'template.yml');

  return await getTpl(builtTplPath);
}
