...
function ensureMountTargetsExist(mountTargets) {
  if (_.isEmpty(mountTargets)) {
    throw new Error(red('Nas has not configured the mount point yet, please go to the console https://nas.console.aliyun.com/ to manually create the mount point.'));
  }
}

function ensureSecurityGroupsExist(securityGroups) {
  if (_.isEmpty(securityGroups)) {
    throw new Error(red('The security group has not been configured, please refer to https://help.aliyun.com/knowledge_detail/72959.html#config'));
  }
}

function ensureNasFileSystemsExist(nasFileSystems) {
  if (_.isEmpty(nasFileSystems)) {
    throw new Error(red(`\nThere is no NAS file system available. You need to handle function dependencies manually to reduce the size of the package. It is recommended that you use NAS or OSS.`));
  }
}
