...
const { readFileFromNasMappings, nasCpFromlocalNasDirToremoteNasDir } = require('./nas/support');
...
async function saveNasMappings(baseDir, nasMappings) {
  ...
  let nasMappingObj = Object.assign({}, nasMappings);
  ...
  if (!_.isEmpty(contentObj)) {
    nasMappingObj = _.mapValues(contentObj, function(value, key) {

      for (const localNasDir of Object.keys(nasMappings)) {
        if (localNasDir === key) {
          value = _.unionWith([...nasMappings[key], ...value], _.isEqual);
        }
      }
      return value;
    });
...
async function updateEnvironmentInTpl(tplPath, tpl, nasFuntionName, envs) {
  ...
  for (const { functionRes } of serviceNeedUpdate) {
    ...
    const mergyedEnvs = _.mergeWith(formerEnvs, envs, customizer);

    if (_.isEmpty(mergyedEnvs)) { continue; }

    functionProp.EnvironmentVariables = mergyedEnvs;
  }

  util.outputTemplateFile(tplPath, updatedTplContent);

  console.log(green(`\nFun add environment variables to your template.yml`));
...
function outputNasMappingLog(baseDir, nasMappingPath, localNasDir) {
  console.log(green(`\n${path.relative(baseDir, localNasDir)} has been correctly added to ${nasMappingPath}`));
}
...
async function processNasAutomationConfiguration({ tpl, tplPath, baseDir, runtime, codeUri,
  ...
  // TODO fun deploy
  console.log(yellow(`Fun has automatically uploaded your code dependency to NAS, please re execute ‘fun deploy’.`));
}

async function backupTemplateFile(tplPath) {
  const newPath = generateBackupTplPath(tplPath);
  await fs.copy(tplPath, newPath);
  console.log(green(`Fun automatically backups the original ${path.basename(tplPath)} file to ${newPath}`));
}

const NODE_RUNTIME = ['nodejs6', 'nodejs8', 'nodejs10'];

async function nasAutomationConfigurationIfNecessary({
  nasFunctionName,
  compressedSize,
  nasConfig,
  vpcConfig,
