...
const { readFileFromNasYml } = require('./nas/support');
...
async function saveNasMappings(baseDir, nasMappings) {
  ...
  const nasMappingsInNasYml = contentObj.nasMappings || {};

  const nasMappingObj = _.mapValues(nasMappings, (nasMapping, key) => {
    for (const localNasDir of Object.keys(nasMappingsInNasYml)) {
      if (localNasDir === key) {
        nasMapping = _.unionWith([...nasMappingsInNasYml[key], ...nasMapping], _.isEqual);
      }
    }
    return nasMapping;
  });

  contentObj.nasMappings = nasMappingObj;
