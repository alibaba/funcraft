...
async function processNasAutoConfigurationForDeploy({ tpl, tplPath, runtime, codeUri, convertedNasConfig,
  nasServiceName,
  nasFunctionName
}) {
  ...
  const rs = await processNasMappingsAndEnvs({ tpl, tplPath, runtime, codeUri, baseDir,
    serviceName: nasServiceName,
    functionName: nasFunctionName,
    convertedNasConfig
  });

  const updatedTplContent = await processJavaIfNecessary({ runtime, baseDir, tplPath,
    tpl: rs.updatedTpl,
    serviceName: nasServiceName,
    functionName: nasFunctionName
  });

  const serviceNasMappings = await processPythonModelIfNecessary({ codeUri, runtime, baseDir,
    serviceName: nasServiceName,
...
async function nasAutoConfigurationForDeploy({ tplPath, runtime, codeUri, nasConfig, vpcConfig,
  compressedSize,
  nasFunctionName,
  nasServiceName
}) {
    ...
    if (await promptForConfirmContinue(`Do you want to let fun to help you automate the configuration?`)) {
      ...
      if (definition.isNasAutoConfig(nasConfig)) {
        ...
        if (yes) {
          ...
          tplChanged = await processNasAutoConfigurationForDeploy({
...
async function processNasAutoConFigurationForPackage({ tpl, tplPath, runtime, codeUri,
  nasServiceName,
  nasFunctionName
}) {
  const baseDir = getBaseDir(tplPath);
  ...
  return rs.updatedTpl;
}

async function nasAutoConfigurationForPackage({ serviceName, functionName,
  tplPath,
  nasConfig,
  runtime,
  codeUri,
  ...
  let stop = false;
  if (compressedSize > 52428800 && _.includes(SUPPORT_RUNTIMES, runtime)) {
    console.log(red(`\nFun detected that your function ${serviceName}/${functionName} sizes exceed 50M. It is recommended that using the nas service to manage your function dependencies.`));
    ...
    if (definition.isNasAutoConfig(nasConfig)) {
      ...
      if (yes) {
        ...
        await processNasAutoConFigurationForPackage({ tpl, tplPath, runtime, codeUri,
    ...
    } else if (_.isEmpty(nasConfig)) {
      const yes = await promptForConfirmContinue(`We recommend using the 'NasConfig: Auto' configuration to manage your function dependencies.`);
      if (yes) {

        await backupTemplateFile(tplPath);
        // write back to yml
        const updatedTpl = await updateNasAutoConfigure(tplPath, tpl, serviceName);

        await processNasAutoConFigurationForPackage({ tpl: updatedTpl, tplPath, runtime, codeUri, nasConfig,
          nasServiceName: serviceName,
          nasFunctionName: functionName
        });

      } else {
        throw new Error('only support NasConfig: Auto.');
      }
      stop = true;
    } else {
...
async function makeFunction(baseDir, {
  ...
  if (!onlyConfig) { // ignore code
    ...
    if (codeUri && codeUri.startsWith('oss://')) { // oss://my-bucket/function.zip
      ...
      const rs = await nasAutoConfigurationForDeploy({
...
module.exports = {
  ...
  generateFunIngore,
  parseMountDirPrefix,
  nasAutoConfigurationForPackage
};