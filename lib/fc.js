...
const { addEnv, resolvePrefixConfForEnv } = require('./install/env');
...
async function getFunCodeAsBase64(baseDir, codeUri, runtime, functionName) {
  if (codeUri) {
    if (codeUri.endsWith('.zip') || codeUri.endsWith('.jar') || codeUri.endsWith('.war')) {
      ...
      return { base64: Buffer.from(await fs.readFile(codeUri)).toString('base64') };
    }
  } else {
    codeUri = './';
  }
...
async function makeFunction(baseDir, {
  ...
  const confEnv = await resolvePrefixConfForEnv(baseDir, codeUri);

  if (!_.isEmpty(confEnv)) {
  ...
  console.log('params:' + JSON.stringify(params.environmentVariables, null, 4));
...
async function getFcUtilsFunctionCode(filename) {
  return await fs.readFile(path.join(__dirname, 'fc-utils', filename));
}
...
async function makeFcUtilsFunctionNasDirChecker(role, vpcConfig, nasConfig) {
  ...
  const functionCode = await getFcUtilsFunctionCode('fc-nas-dir-check.js');
