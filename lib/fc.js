...
async function makeFunction(baseDir, {
  ...
  if (!onlyConfig) { // ignore code
    ...
    if (codeUri && codeUri.startsWith('oss://')) { // oss://my-bucket/function.zip
      ...
      const { base64, count, size } = await getFunCodeAsBase64(baseDir, codeUri, runtime, functionName);
      console.log(green(`\t\tThe function ${functionName} has been packaged. A total of ` + yellow(`${count}`) + `${count === 1 ? ' file' : ' files'}` + ` files were compressed and the final size was` + yellow(` ${size}`)));
      code = {
        zipFile: base64
      };
    }
