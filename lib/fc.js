...
const { getFcClient, getEcsPopClient } = require('./client');
const { addEnv, resolveLibPathsFromLdConf } = require('./install/env');
const { readFileFromNasMappings } = require('./nas/support');
...
const NAS_MAPPING_PATH = path.join('.fun', 'nasMappings.json');

const SYSTEM_DEPENDENCY_PATH = path.join('.fun', 'root');

const NODE_RUNTIME = ['nodejs6', 'nodejs8', 'nodejs10'];
...
async function saveNasMappings(baseDir, nasMappings) {
  ...
  const contentObj = await readFileFromNasMappings(baseDir);

  const nasMappingObj = _.mapValues(nasMappings, (nasMapping, key) => {

    for (const localNasDir of Object.keys(contentObj)) {
      if (localNasDir === key) {
        nasMapping = _.unionWith([...contentObj[key], ...nasMapping], _.isEqual);
      }
    }
    return nasMapping;
  });

  await fs.outputFile(nasMappingPath, JSON.stringify(nasMappingObj, null, 4));

  return nasMappingObj;
}

async function updateEnvironmentsInTpl(tplPath, tpl, nasFuntionName, envs) {

  const updatedTplContent = _.cloneDeep(tpl);

  const serviceNeedUpdate = [];
  definition.iterateFunctions(updatedTplContent, (serviceName, serviceRes, functionName, functionRes) => {
    if (nasFuntionName === functionName) {
      serviceNeedUpdate.push({
        functionRes
      });
    }
  });

  for (const { functionRes } of serviceNeedUpdate) {
    ...
    if (_.isEmpty(mergedEnvs)) { continue; }

    functionProp.EnvironmentVariables = mergedEnvs;
  }
...
function updateNasAutoConfigureInTpl(tplPath, tpl, nasServiceName) {
  ...
  for (const { serviceRes } of serviceNeedUpdate) {
...
async function generateNasMappingsAndEnvs({
  ...
  const prefix = await getRemoteNasDirPrefix(nasConfig);
  // used for log
  const nasMappingPath = path.resolve(baseDir, NAS_MAPPING_PATH);
  ...
  if (await fs.pathExists(localDependencyPath)) { // language dependence
    ...
    Object.assign(envs, generateLanguageNasEnvs(remoteNasDir));
...
async function getRemoteNasDirPrefix(nasConfig) {
  ...
  if (nasConfig.MountPoints.length === 1) {

    result = _.head(mountPoints).MountDir;
  } else {
    // select nas
    const answer = await promptForNasSelection(nasConfig.MountPoints);

    result = answer.mountDir;
  }
...
function generateLanguageNasEnvs(languageEnv) {
  // TODO  other runtime
  return {
    'NODE_PATH': languageEnv
  };
}
...
async function nasCpFromlocalNasDirToRemoteNasDir(tpl, tplPath, baseDir, nasServiceName, nasMappings) {
  ...
  for (const { localNasDir, remoteNasDir } of nasMappings) {
    ...
    try {
      await nasCp(srcPath, dstPath, true, false, localNasTmpDir, tpl, tplPath, baseDir, false);
    } catch (error) {
...
async function processNasAutomationConfiguration({ tpl, tplPath, baseDir, runtime, codeUri,
  ...
  const { serviceRes, functionRes } = definition.findFunctionInTpl(`${nasServiceName}/${nasFunctionName}`, tpl);
  ...
  await updateEnvironmentsInTpl(tplPath, tpl, nasFunctionName, envs);
  // fun nas cp
...
async function nasAutomationConfigurationIfNecessary({ tplPath, runtime, baseDir, codeUri, nasConfig, vpcConfig,
  ...
  let stop = false;
  if (compressedSize > 1320 && _.includes(NODE_RUNTIME, runtime)) { // 50M
    ...
    if (await promptForConfirmContinue(`Do you want to let fun to help you automate the configuration?`)) {
      ...
      if (definition.isNasAutoConfig(nasConfig)) {
        ...
        if (yes) {
          ...
          // list available NAS
          const { mountTarget, securityGroupId } = await continuousSelectionNAS();
...
async function continuousSelectionNAS() {
  const nasFileSystems = await nas.getAvailableNasFileSystems();
  ensureNasFileSystemsExist(nasFileSystems);

  const nasAnswer = await promptForFileSystems(nasFileSystems);
  const nasSelected = nasFileSystems.filter(f => f.description === nasAnswer.description);
