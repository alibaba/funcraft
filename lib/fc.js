...
async function generateDefaultLogConfig() {
  const profile = await getProfile();
  return {
    project: `fun-gen-${profile.defaultRegion}-${profile.accountId}`,
    logstore: `fun-gen-${profile.defaultRegion}-${profile.accountId}`
  };
}
...
module.exports = {
  ...
  makeFcUtilsFunctionNasDirChecker, generateDefaultLogConfig
};