...
async function detectLibrary(codeUri, runtime, baseDir, wrap = '', functionName) {
  const stats = await lstat(codeUri);
  if (stats.isFile()) {
    let libraryFolders = runtimeTypeMapping[runtime];
    let absoluteCodePath;

    if (path.isAbsolute(codeUri)) {
      absoluteCodePath = codeUri;
    } else {
      absoluteCodePath = path.join(baseDir, codeUri);
    }
...
async function getFunCodeAsBase64(baseDir, codeUri, runtime, functionName) {
  if (codeUri) {
...
async function makeFunction(baseDir, {
  ...
  if (codeUri && codeUri.startsWith('oss://')) {
    ...
    const { base64, count } = await getFunCodeAsBase64(baseDir, codeUri, runtime, functionName);
...
async function makeService({
  ...
  if (!_.isEmpty(vpcConfig) || _.isEqual(nasConfig, 'Auto')) {

    if (_.isEqual(vpcConfig, 'Auto') || (_.isEmpty(vpcConfig) && _.isEqual(nasConfig, 'Auto'))) {
  ...
  if (_.isEqual(nasConfig, 'Auto')) {

    const vpcId = vpcConfig.vpcId;
    const vswitchId = _.head(vpcConfig.vswitchIds);

    console.log("\tusing 'NasConfig: Auto', will auto generate related nas file system");
...
async function ensureNasDirExist({
  ...
  const dirsNeedToCheck = mapMountPointDir(mountPoints, (mountPointDomain, remoteDir, mountDir) => {
    if (remoteDir !== '/') {
      return path.join(mountDir, remoteDir);
    } return null;
  });
...
async function getFcUtilsFunctionCode(filename) {
  const functionCode = await readFile(path.join(__dirname, 'fc-utils', filename));
...
async function makeFcUtilsFunctionNasDirChecker(role, vpcConfig, nasConfig) {
  ...
  const codes = await getFcUtilsFunctionCode('fc-nas-dir-check.js');
