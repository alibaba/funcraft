...
async function updateEnvironmentsInTpl(tplPath, tpl, functionProp, envs) {
  const updatedTplContent = _.cloneDeep(tpl);

  if (_.isEmpty(functionProp)) { return updatedTplContent; }
  ...
  if (_.isEmpty(mergedEnvs)) { return updatedTplContent; }
...
async function processNasAutomationConfiguration({ tpl, tplPath, baseDir, runtime, codeUri, convertedNasConfig,
  ...
  const { serviceRes, functionRes } = definition.findFunctionByServiceAndFunctionName(tpl.Resources, nasServiceName, nasFunctionName);
  ...
  await updateEnvironmentsInTpl(tplPath, tpl, functionProp, envs);
  // fun nas cp
