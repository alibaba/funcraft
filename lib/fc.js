...
async function generateFunIngore(baseDir, codeUri, tplPath) {
  ...
  return await funignore(baseDir, tplPath);
}
...
async function zipCode(baseDir, codeUri, runtime, functionName, tplPath) {
  let codeAbsPath;
  ...
  const ignore = await generateFunIngore(baseDir, codeAbsPath, tplPath);
...
async function updateEnvironmentsInTpl({ tplPath, tpl, envs,
  ...
  console.log(green(`Fun add environment variables to your ${tplPath}`));
...
function resolveLocalNasDir(runtime, baseDir, codeUri, localDirInNasMappings, serviceName, functionName) {
  let localDir;
  if (runtime === 'java8') {
    localDir = path.join(baseDir, localDirInNasMappings, serviceName, functionName, 'lib');
  } else {
    localDir = path.join(codeUri, localDirInNasMappings);
  }
  return localDir;
}
...
async function updateEnvironments({
  ...
  if (path.dirname(tplPath) !== path.resolve(baseDir)) {

    const originTpl = await getTpl(originTplPath);

    await updateEnvironmentsInTpl({
      tplPath: originTplPath,
      tpl: originTpl,
      envs,
...
async function makeFunction(baseDir, {
  ...
  if (!onlyConfig) { // ignore code
    ...
    if (codeUri && codeUri.startsWith('oss://')) { // oss://my-bucket/function.zip
      ...
      const { base64, count, compressedSize } = await zipCode(baseDir, codeUri, runtime, functionName, tplPath);
...
async function makeService({
  ...
  if (isNasAuto) {
    ...
    nasConfig = await nas.generateAutoNasConfig(serviceName, vpcId, vswitchId, nasConfig.UserId, nasConfig.GroupId);
