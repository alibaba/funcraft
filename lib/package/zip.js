...
async function zipFolder(zip, folder, folders, funignore) {
  ...
  await Promise.all((await readdir(dir)).map(async (f) => {
    ...
    if (s.isFile()) {
      ...
      let countObjChild = await zipFolder(zFolder, f, folders.slice(), funignore);
      countObj.count += countObjChild;
...
exports.pack = async function (file, funignore) {
  ...
  if (stats.isFile()) {
    ...
    filesCount = await zipFolder(zip, file, [], funignore);
  } else {
