...
exports.pack = async function (file, funignore) {
  ...
  zipArchiver.pipe(output).on('close', () => {
    ...
    compressedSize = bytes(zipArchiver.pointer(), {
      unitSeparator: ' '
    });

    if (zipArchiver.pointer() > 52428800) { // 50M
      console.log(`We detected that the compressed size of your code package is ${compressedSize}`);
    }
  });
