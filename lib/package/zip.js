...
async function zipFolder(zipArchiver, folder, folders, funignore) {
  folders.push(folder);
  ...
  await Promise.all((await readdir(dir)).map(async (f) => {
    ...
    if (s.isFile()) {
      count++;
      zipArchiver.append(fPath, {'name': fPath});
    } else if (s.isDirectory()) {

      let folderFilesCount = await zipFolder(zipArchiver, f, folders.slice(), funignore);

      count += folderFilesCount;
    } else if (s.isSymbolicLink()) {

      zipArchiver.symlink(fPath, fPath);
    }  
  }));
  return count;
}
...
exports.pack = async function (file, funignore) {
  ...
  if (!await fsExtra.pathExists(randomDir)) {
    await fsExtra.ensureDir(randomDir);
  }
  ...
  output.on('close', () => {
    if (zipArchiver.pointer() > 52428800) { // 50M
      ...
      console.log(`We detected that the compressed size of your code package is ${formatFileSize(zipArchiver.pointer())}`);
    }
  });
  ...
  if (stats.isFile()) {
    ...
    zipArchiver.append(fs.createReadStream(file), {'name': file});

    count++;
  } else if (stats.isDirectory()) {
    debug('append folder: %s, absolute path is %s', file, path.resolve(file));

    count = await zipFolder(zipArchiver, file, [], funignore);
  } else {
  ...
  await fsExtra.remove(randomDir);
