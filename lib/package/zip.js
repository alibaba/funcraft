...
async function packTo(file, funignore, targetPath) {
  ...
  zipArchiver.pipe(output).on('close', () => {
    compressedSize = zipArchiver.pointer();
    if (compressedSize > 52428800) { // 50M
      console.log(`We detected that the compressed size of your code package is ${bytes(compressedSize)}`);
    }
  });
...
async function pack(file, funignore) {
  ...
  const count = await packTo(file, funignore, zipPath);
