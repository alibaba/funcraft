...
async function processNasAutoToRosTemplate(ossClient, tpl) {
...
async function pack(tplPath, bucket, outputTemplateFile) {
  ...
  const updatedFlowTpl = await transformFlowDefinition(baseDir, updatedTpl);
  ...
  await uploadLocalNasDir(ossClient, baseDir, mergedNasMapping);
  const updatedRosTpl = await processNasAutoToRosTemplate(ossClient, updatedFlowTpl);
