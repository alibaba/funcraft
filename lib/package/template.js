...
async function processNasPythonPaths(tpl, tplPath) {
  ...
  for (const { serviceRes, functionRes } of definition.findFunctionsInTpl(updatedTplContent)) {
    ...
    if (!_.includes(['python2.7', 'python3'], runtime)) { continue; }
  ...
  outputTemplateFile(tplPath, updatedTplContent);
