...
async function zipCodeToOss({ ossClient, codeUri, runtime, ignore, tplPath, nasConfig,
  ...
  const stop = await fc.autoConfigurationForNasAuto({ serviceName, functionName,
    tplPath,
    nasConfig,
    runtime,
    codeUri,
...
async function uploadAndUpdateFunctionCode({
  ...
  do {
    ...
    for (const { serviceName, serviceRes, functionName, functionRes } of functionsNeedUpload) {
