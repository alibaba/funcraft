...
async function uploadAndUpdateFunctionCode(baseDir, tpl, ossClient) {
  ...
  for (const { functionRes } of functionsNeedUpload) {
    ...
    const codeUriCaches = codeUriCache.map(cache => cache.cacheCodeUri);

    if (_.includes(codeUriCaches, absCodeUri)) {

      for (const cache of codeUriCache) {
        if (absCodeUri === cache.cacheCodeUri) {
          functionRes.Properties.CodeUri = cache.resolveCodeUri;
          break;
        }
      }
    } else {
