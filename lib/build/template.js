...
const debug = require('debug')('fun:build:template');

function findBuildFuncs(buildName, tpl) {
  if (buildName) {
    const [parsedServiceName, parsedFunctionName] = definition.parseInvokeName(buildName);

    debug(`parse service name ${parsedServiceName}, functionName ${parsedFunctionName}`);

    const func = definition.findFunctionInTpl(parsedServiceName, parsedFunctionName, tpl);
