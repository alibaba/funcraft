...
function showLocalStartNextTips(customDomains) {
  ...
  // const startCommand = customDomains.map((cur, index) => `${index + 1}.fun local start ${cur}`);
  // const debugCommand = customDomains.map((cur, index) => `${index + 1}.fun local start -d 3000 ${cur}`);

  const startTip = `${customDomains.join('\n*')}`;
  const debugTip = `${customDomains.join('\n*')}`;
