...
function parseParameters(parametersInTpl, parameterOverride) {

  if (!_.isEmpty(parameterOverride) && !_.isEmpty(parametersInTpl)) {
    let pairs = 1;

    return _.transform(parameterOverride, (result, value, key) => {
      if (!_.includes(Object.keys(parametersInTpl), key)) {
        throw new Error(`Incorrect parameters: ${key}`);
      }
      result[`Parameters.${pairs}.ParameterKey`] = key;
      result[`Parameters.${pairs}.ParameterValue`] = value;
      pairs++;
    }, {});
  }
}

module.exports = { funymlToFunfile, funfileToDockerfile, resolveEnv, parsePairs, parseParameters };