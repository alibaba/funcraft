...
async function recordMetaData(baseDir, functions, tplPath, metaPath, buildOps) {
  ...
  const codeUris = functions.map(func => {
    ...
    const { serviceName, functionName, functionRes } = func;
    ...
    if (codeUri.endsWith('.zip') || codeUri.endsWith('.jar') || codeUri.endsWith('.war')) {
      const relative = path.relative(asbBaseDir, absCodeUri);

      console.warn(red(`\nDetectionWarning: your codeuri is '${relative}', and 'fun build' will not compile your functions. It is recommended that you modify ${serviceName}/${functionName}'s 'CodeUri' property to the directory where 'pom.xml' is located.`));
    }

    return absCodeUri;
  });

  const pomPaths = codeUris
    .map(absCodeUri => { return path.join(absCodeUri, 'pom.xml'); })
    .filter(pomPath => { return fs.pathExistsSync(pomPath); });

  await recordMtimes([...codeUris, ...[tplPath], ...pomPaths], buildOps, metaPath);
}
