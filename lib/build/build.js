...
async function checkFunfileRuntime(funfilePath, runtime) {
  const funfilePathContent = await file.readLines(funfilePath);

  const runtimeLine = funfilePathContent.find(line => {
    return line.indexOf('RUNTIME') !== -1;
  });

  if (!runtimeLine) { return; }

  if (_.trim(_.trimStart(_.trim(runtimeLine), 'RUNTIME')) !== runtime) {
    console.warn(red(`\nDetectionWarning: Fun has detected that the 'runtime' in your template.yml is inconsistent with that in Funfile.\n`));
  }
}
...
async function buildFunction(buildName, tpl, baseDir, useDocker, stages, verbose, tplPath) {
  ...
  for (let func of buildFuncs) {
    ...
    if (funfilePath) {
      await checkFunfileRuntime(funfilePath, runtime);
      imageTag = await processFunfile(serviceName, serviceRes, asbCodeUri, funfilePath, baseDir, funcArtifactDir);
    }
