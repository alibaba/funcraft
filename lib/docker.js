...
const { getMountPathsNotSharedToDocker } = require('./docker-support');
...
async function generateDockerEnvs(functionProps, debugPort, httpParams, nasConfig, ishttpTrigger) {
  ...
  const confEnv = await resolveLibPathsFromLdConf(path.dirname(tplPath), functionProps.CodeUri);
...
async function createContainer(opts) {
  ...
  if (opts && isMac) {
    if (opts.HostConfig) {
      const mountPathsNotSharedToDocker = await getMountPathsNotSharedToDocker(opts.HostConfig.Mounts);
      if (isMac && mountPathsNotSharedToDocker.length > 0) {
        throw new Error(red(`Please add ${mountPathsNotSharedToDocker} to Docker File sharing list, more information please refer to https://github.com/alibaba/funcraft/blob/master/docs/usage/faq-zh.md`));
      }
    }
  }
  ...
  try {
    ...
    if (ex.message.indexOf('invalid mount config for type') !== -1 && isWin) {
      throw new Error(red(`Maybe Docker Shared Drivers does not contain ${bindSourcePath}, Please make sure ${bindSourcePath} is in Docker Shared Drivers. If you want to share your local drives with Docker Desktop, please refer to https://docs.docker.com/docker-for-windows/#shared-drives`));
    }
    throw ex;
  } 
  return container;
