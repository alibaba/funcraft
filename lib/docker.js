...
async function pullImage(imageName) {
  ...
  const resolveImageName = await dockerOpts.resolveImageNameByRegistry(imageName);
  ...
  return new Promise((resolve, reject) => {
    ...
    const onFinished = async (err) => {
      ...
      if (resolveImageName.indexOf(dockerOpts.ALIYUN_REGISTRY) === 0) {
        const shortImageName = resolveImageName.slice(dockerOpts.ALIYUN_REGISTRY.length);
        // rename
        await stream.tag({
