...
const fcErrorTransform = require('./fc-error-transform');
...
async function resolveNasConfigToMounts(serviceName, nasConfig, nasBaseDir) {
  ...
  return nasMappings.map(nasMapping => {
    debug('mounting local nas mock dir %s into container %s\n', nasMapping.localNasDir, nasMapping.remoteNasDir);
    return {
      Type: 'bind',
      Source: nasMapping.localNasDir,
...
async function run(opts, event, outputStream, errorStream, context = {}) {
  ...
  const errorTransform = fcErrorTransform({
...
async function startContainer(opts, outputStream, errorStream, context = {}) {
  ...
  if (logs) {
    ...
    container.modem.demuxStream(logStream, outputStream, fcErrorTransform({
      serviceName: context.serviceName,
      functionName: context.functionName,
      errorStream
    }));
...
function displaySboxTips(runtime) {
  ...
  console.log(yellow(`You can install system dependencies in the following way:`))
  ...
  switch(runtime) {
    ...
    case 'nodejs8':
    case 'nodejs10':
      console.log(yellow(`You can install node modules in the following way:`));
      console.log(yellow(`fun-install npm install puppeteer\n`));
      break;
    case 'python2.7':
    case 'python3':
      console.log(yellow(`You can install pip dependencies in the following way:`));
      console.log(yellow(`fun-install pip install flask`));
      break;
    default:
      break;
  }
  console.log(yellow('type \'fun-install --help\' for more documentation\n'));
}
...
async function detectDockerVersion(serverVersion) {
  ...
  if (Number.parseInt(cur[0]) === 1 && Number.parseInt(cur[1]) <= 13) {
    throw new Error(`We detected that your docker version is ${serverVersion}, for a better experience, please upgrade the docker version.`);
  }
}
...
module.exports = {
  ...
  renameContainer, detectDockerVersion
};