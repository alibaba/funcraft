...
function generateDockerCmd(functionProps) {
...
async function generateDockerOpts(runtime, containerName, mount, cmd, debugPort, envs) {
...
async function showDebugIdeTips(serviceName, functionName, runtime, mount, debugPort) {
...
async function invokeFunction(serviceName, functionName, functionDefinition, debugPort, event, debugIde, httpParams, outputStream, errorStream) {
  ...
  if (debugPort && debugIde) {
    await showDebugIdeTips(serviceName, functionName, runtime, mount, debugPort);
  }

  const containerName = generateRamdomContainerName();

  const envs = await generateDockerEnvs(functionProps, debugPort, httpParams);
  const opts = await generateDockerOpts(runtime, containerName, mount, cmd, debugPort, envs);
