...
async function resolveNasConfigToMounts(serviceName, nasConfig, tplPath) {
  ...
  if (isNasAuto) { // support 'NasConfig: Auto'
    ...
    const nasDir = path.join(tplFolder, '.fun', 'nas', 'auto-default');

    const nasMountDir = path.join(nasDir, serviceName);

    if (!await fs.pathExists(nasMountDir)) {
      await mkdirp(nasMountDir);
    }
  ...
  if (mountPoints) {
    ...
    for (let mountPoint of mountPoints) {
      ...
      const { mountSource, mountDir, serverPath, serverAddr } = nas.resolveMountPoint(mountPoint);
...
function resolveInstallTargetsToMounts(installTargets) {
...
async function generateDockerEnvs(functionProps, debugPort, httpParams, nasConfig) {
  const envs = {};
...
async function startInstallationContainer({ runtime, imageName, codeUri, targets }) {
  ...
  const installMounts = resolveInstallTargetsToMounts(targets);
