...
async function againImageExit(imageName) {

  let replaceImageName;

  if (imageName.indexOf(dockerOpts.ALIYUN_REGISTRY) !== -1) {

    replaceImageName = imageName.slice(0, 33);
    
  } else {
    replaceImageName = dockerOpts.ALIYUN_REGISTRY + '/' + imageName;
  }

  const images = await docker.listImages({
    filters: {
      reference: [replaceImageName]
    }
  });
...
async function pullImageIfNeed(imageName) {
  ...
  if (!exist || !skipPullImage) {
    if (await againImageExit(imageName)) { return; }
