...
async function startInstallationContainer({ runtime, imageName, codeUri, containerName }) {
  debug(`runtime: ${runtime}`);
  debug(`codeUri: ${codeUri}`);

  if (!imageName) {
    imageName = dockerOpts.resolveRuntimeToDockerImage(runtime, false);
    if (!imageName) {
