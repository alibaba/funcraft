      ...
      for (const r of dockerOpts.DOCKER_REGISTRIES) {
        if (resolveImageName.indexOf(r) === 0) {
          ...
          await image.tag({
            ...
            repo: resolveImageName.slice(r.length + 1) // short name
          });
          break;
        }
      }
...
async function generateDockerEnvs(baseDir, functionProps, debugPort, httpParams, nasConfig, ishttpTrigger, debugIde, debugArgs) {
...
async function detectDockerVersion() {
  ...
  const currentVersion = await docker.info().ServerVersion;
  let cur = currentVersion.split('.');
  // 1.13.1
  if (Number.parseInt(cur[0]) === 1 && Number.parseInt(cur[1]) <= 13) {
    console.warn(`We detected that your docker version is ${currentVersion}, for a better experience, please upgrade the docker version.`);
  }
}
