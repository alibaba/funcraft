...
async function getImageIfExist(imageName) {
  let otherImageName;

  if (imageName.indexOf(dockerOpts.ALIYUN_REGISTRY) !== -1) {
    otherImageName = imageName.slice(33);
  } else {
    otherImageName = dockerOpts.ALIYUN_REGISTRY + '/' + imageName;
  }

  const images = await docker.listImages({
    filters: {
      reference: [imageName, otherImageName]
    }
  });
...
async function pullImageIfNeed(imageName) {
  const localImageName = await getImageIfExist(imageName);

  if (!localImageName || !skipPullImage) {
    ...
    if (imageName.indexOf(dockerOpts.ALIYUN_REGISTRY) !== -1) {
      ...
      await image.tag({
        name: imageName,
        repo: imageName.slice(33)
      });

      return imageName.slice(33);
    }

    return imageName;
  }

  debug(`skip pulling image ${localImageName}...`);
...
async function startInstallationContainer({ runtime, imageName, codeUri, targets }) {
  ...
  const resolveImageName = await pullImageIfNeed(imageName);

  const opts = dockerOpts.generateInstallOpts(resolveImageName, mounts, envs);
...
module.exports = {
  getImageIfExist, generateDockerCmd,
  pullImage,
