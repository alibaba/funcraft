...
const { addEnv, addInstallTargetEnv, resolvePrefixConfForEnv } = require('./install/env');
...
async function generateDockerEnvs(functionProps, debugPort, httpParams, nasConfig, ishttpTrigger) {
  ...
  const confEnv = await resolvePrefixConfForEnv(path.dirname(tplPath), functionProps.CodeUri);
