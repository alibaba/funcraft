...
async function deployNasService(baseDir, tpl) {
  ...
  const zipCodePath = path.resolve(__dirname, '../fc-utils/fc-fun-nas-server/dist/fun-nas-server.zip');

  if (!await fs.pathExists(zipCodePath)) {
    throw new Error('could not find ../fc-utils/fc-fun-nas-server/dist/fun-nas-server.zip');
  }
