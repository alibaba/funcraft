...
const { getNasYmlPath, getRootBaseDir } = require('../tpl');
...
async function extractNasMappingsFromNasYml(baseDir, serviceName) {
  ...
  console.log('rootBaseDir:' + rootBaseDir);
...
async function generateIgnoreFileFromNasYml(tplPath) {

  if (!tplPath) { return []; }

  const nasYmlPath = getNasYmlPath(tplPath);
