...
function jsonParse(str) {
  ...
  try {
    ...
    console.error(red(`Unable to parse JSON file ${str}. Error: ${err}`));
  }
  return obj;
}

async function readFileFromNasMappings(baseDir) {
  const NAS_MAPPING_PATH = path.join('.fun', 'nasMappings.json');

  const nasMappingPath = path.resolve(baseDir, NAS_MAPPING_PATH);
  if (!await fs.pathExists(nasMappingPath)) {
    return {};
  }
  const content = await fs.readFile(nasMappingPath, 'utf8');
...
async function getNasMappingsFromFile(baseDir, nasServiceName) {
  const content = await readFileFromNasMappings(baseDir);
  if (_.isEmpty(content)) {
    return [];
  }
  if (_.isEmpty(content[nasServiceName])) {
    return [];
  }
  return content[nasServiceName];
}

async function generateIgnoreFileFromNasMappings(baseDir) {
  ...
  _.forEach(content, (nasMappings, key) => {
    for (const nasMapping of nasMappings) {
      ignoreList.push(path.relative(baseDir, nasMapping.localNasDir));
    }
  });
...
module.exports = {
  ...
  readFileFromNasMappings,
  generateIgnoreFileFromNasMappings,
  mergyNasMappings,
  getNasMappingsFromFile,
  nasCpFromlocalNasDirToremoteNasDir
};
