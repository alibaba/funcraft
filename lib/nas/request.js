...
async function checkHasUpload(dstPath, commonPath, fileHashValue, fileName, isDir, isEndWithSlash) {
  const urlPath = commonPath + 'nas/stats';
...
async function sendMergeRequest(commonPath, nasTmpDir, dstDir, fileName, fileHashValue) {
  const urlPath = commonPath + 'split/merge';
...
async function uploadSplitFile(commonPath, nasTmpDir, filePath) {
  ...
  const urlPath = commonPath + 'split/uploads';
...
async function uploadFile(srcFilePath, dstDir, commonPath, fileHashValue, fileName) {
  console.log('Uploading...');
  const urlPath = commonPath + 'uploads';
