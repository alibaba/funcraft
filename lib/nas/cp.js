...
async function cp(srcPath, dstPath, recursive, noClobber, localNasTmpDir, tpl, tplPath, baseDir, isSync, noTargetDirectory = false) {
  ...
  if (isNasProtocol(srcPath) && !isNasProtocol(dstPath)) {
    ...
    var actualDstPath = await checkCpDstPath(srcStats, dstStats, recursive, noClobber, nasHttpTriggerPath, noTargetDirectory);

    if (isSync && srcPathIsDir) { actualDstPath = resolvedDst; }
...
function checkCpSrcPath(srcStats, recursive) {
  const { srcPath, srcPathExists, srcPathIsDir } = srcStats;
...
async function checkCpDstPath(srcStats, dstStats, recursive, noClobber, nasHttpTriggerPath, noTargetDirectory = false) {
