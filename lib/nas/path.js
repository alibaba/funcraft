...
function parseNasPath(nasPath) {
  ...
  const idxNext = mid.indexOf(splitNext);
  
  // idxNext === 0 时，nasPath 格式为 nas:///mnt/nas ，其中 /mnt/nas 是挂载目录
  if (idx === -1 && idxNext === -1) {
    throw new Error('nas path err: ' + nasPath);
  }
  ...
  var service;
  if (idx !== -1) {
    resolvedNasPath = path.posix.join('/', mid.substr(idx + split.length));
  
    service = mid.substr(0, idx);
  } else if (idxNext !== -1) {
    resolvedNasPath = path.posix.join('/', mid.substr(idxNext + splitNext.length));
  
    service = mid.substr(0, idxNext);
  }
