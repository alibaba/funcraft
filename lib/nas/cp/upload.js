...
async function upload(srcPath, dstPath, nasHttpTriggerPath) {
  ...
  // 如果是大于 5M 的文件，会被拆分，放到 .fun_nas_tmp 的目录
  ...
  const uploadedSplitFilesHashMap = jsonToMap(uploadedSplitFilesHash);
  if (files.length === 1) { // file size is less than 5M
    ...
    const needToUploadfiles = await splitFiles(uploadedSplitFilesHashMap, files);
