...
function splitFile(filePath, maxFileSize, outputPath) {
  return new Promise((resolve, reject) => {
    let splitFilePathArr = [];
    splitFileFunc.splitFileBySize(filePath, maxFileSize)
      .then((splitedFilePaths) => {
        Promise.all(splitedFilePaths.map((splitedFilePath) => {
          let fileName = path.basename(splitedFilePath);
          let splitedFileDstPath = path.join(outputPath, fileName);
          fs.rename(splitedFilePath, splitedFileDstPath, (err) => {
            if (err) {
              reject(err);
              return;
            }
            splitFilePathArr.push(splitedFileDstPath);
          });
        }))
          .then(() => {
            resolve(splitFilePathArr);
          })
          .catch((err) => {
            reject(err);
          });
