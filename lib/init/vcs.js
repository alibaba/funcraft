...
function cloneArguments(repoType, repoUrl, tail) {
  switch (repoType) {
  case 'git':
    return ['clone', '--depth=1', repoUrl, tail];
  default:
    return ['clone', repoUrl, tail];
  }
}
...
async function clone(repoUrl, cloneToDir = '.', checkout) {
  ...
  spawnSync(repoType, cloneArguments(repoType, repoUrl, tail), { cmd: repoDir, stdio: 'inherit' });
