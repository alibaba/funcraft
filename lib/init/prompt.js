...
async function promptForFileSystems(fileSystems) {
  ...
  let choicesNas = fileSystems.map(m => `(FileSystem)${m.description}, (StorageType)${m.storageType}, (Availability Zone)${m.zoneId}`);
  ...
  return await inquirer.prompt([{
    ...
    filter: (val) => {
      const colonSplit = _.split(val, ',');
      return _.trimStart(colonSplit[0], '(FileSystem)');
    }
  }]);
}

async function promptForNasSelection(mountPoints) {
