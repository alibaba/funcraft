...
function checkPathServer(req, res) {
  ...
  fs.lstat(nasDstPath, (err, stats) => {
    console.log(`${nasDstPath} not exists`);
    if (err) {
      res.send({
        path: nasDstPath,
        isExist: false,
...
function uploadSplitFileServer(req, res) {
  ...
  const fileHashValue = req.query.fileHashValue;
  cnt++;
  console.log('====uuid123_cnt123 : ' + uuid + '_' + cnt);
  
  getRawBody(req, function (err, body) {
    if (err) {
      console.log(` get raw body error : ${err}`);
      res.send({
        stat: 0,
        desc: `Function getRawBody error `
      });
    } else {
      ...
      writeBufToFile(dstSplitFile, body).then(() => {
        getFileHash(dstSplitFile).then((nasSplitFileHash) => {
          if (nasSplitFileHash === fileHashValue) {
            res.send({
              stat: 1,
              desc: `${fileName} send success, uuid : ${uuid}, cnt : ${cnt}`
            });
          } else {
            res.send({
              stat: 0,
          ...
          console.log(`${fileName} hash err : ${err}`);
          res.send({
            stat: 0,
