...
const { writeBufToFile, getFileHash } = require('../../../fc-utils/fc-fun-nas-server/lib/file');
...
describe('POST /commands test', () => {
  const dirPath = `${os.homedir()}/.fc-fun-nas-test-dir/`;
  beforeEach(async () => {
...
describe('GET /stats test', () => {
  const dirPath = `${os.homedir()}/.stats/`;
  beforeEach(async () => {
    await mkdirp(dirPath);
  });
  afterEach(() => {
...
describe('POST /split/merge test', () => {
  const nasTmpDir = `${os.homedir()}/.tmp/`;
  const filePath = `${os.homedir()}/.fun-nas-test-file`;
  const dstDir = `${os.homedir()}/.dst/`;
  ...
  beforeEach(async () => {
    ...
    await mkdirp(dstDir);
    await writeFile(filePath, new Buffer(10 * 1024 * 1024));
...
describe('POST /uploads test', () => {
  const fileBuf = new Buffer('this is a test');
  const dstDir = `${os.homedir()}/.upload/`;
  const fileName = 'nas';
  ...
  it('successful upload test', (done) => {
    ...
    console.log('====test' + fileHashValue);
    const query = {
      dstDir, 
      fileHashValue, 
      fileName
...
describe('POST /split/uploads', () => {
  const nasTmpDir = `${os.homedir()}/.split-tmp/`;
  const fileName = 'nas';
  let fileHashValue;
  
  const body = new Buffer('this is a test');
  beforeEach(async () => {
    await mkdirp(nasTmpDir);
  });
  ...
  it('hash change test', (done) => {
    ...
    request.post('/split/uploads')
      ...
      .expect(
        {
          error: 'split file hash changes, you need to re-sync'
        }, done);
  });
}); 
...
describe('GET /nas/stats', () => {
  ...
  const fileName = 'nas';
  const dstDir = `${os.homedir()}/.nasStat/`;
