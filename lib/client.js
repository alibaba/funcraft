...
const getOssClient = async (bucket) => {
  ...
  debug('query bucket %s region', bucket);

  const location = await queryRegionClient.getBucketLocation(bucket);
...
const getPopROAClient = async (endpoint, apiVersion) => {
  const profile = await getProfile();

  return new Pop.ROAClient({
    endpoint: endpoint,
    apiVersion: apiVersion,
    accessKeyId: profile.accessKeyId,
    accessKeySecret: profile.accessKeySecret,
