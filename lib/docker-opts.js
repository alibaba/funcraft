...
async function generateContainerBuildOpts(runtime, containerName, mounts, cmd, envs, preferredImage) {
  ...
  let imageName = await resolveRuntimeToDockerImage(runtime, true);

  if (imageName.indexOf(ALIYUN_REGISTRY) !== -1) {

    imageName = imageName.slice(33);
  }
...
async function generateLocalInvokeOpts(runtime, containerName, mounts, cmd, debugPort, envs, dockerUser, debugIde, imageName) {
...
async function generateLocalStartRunOpts(runtime, name, mounts, cmd, debugPort, envs, dockerUser, debugIde, imageName) {
