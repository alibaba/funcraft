...
function pathTransformationToVirtualBox(source) {
...
function detectedToolBoxAndPathTransformation(mounts) {
  ...
  if (Array.isArray(mounts)) {
    return mounts.map(element => {

      return replaceSourcePathInMounts(element);
    });
  } 
  return replaceSourcePathInMounts(mounts);
}

function replaceSourcePathInMounts(mountsObj) {

  if (!_.isEmpty(mountsObj)) {

    const replaceMounts = Object.create(mountsObj);
    replaceMounts.Source = pathTransformationToVirtualBox(mountsObj.Source);
...
module.exports = {
  ...
  resolveDockerImageRepo, detectedToolBoxAndPathTransformation
};