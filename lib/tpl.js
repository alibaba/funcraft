...
function detectNasBaseDir(tplPath) {
  ...
  if (idx !== -1) {
    nasPathPrefix = tplPath.substring(0, idx);
    if (!nasPathPrefix) {
      nasPathPrefix = process.cwd();
    }
  } else {
    nasPathPrefix = path.resolve(path.dirname(tplPath));
  }

  return path.join(nasPathPrefix, DEFAULT_NAS_PATH_SUFFIX);
}

module.exports = {
  getTpl, detectTplPath, validateYmlName, detectNasBaseDir, DEFAULT_BUILD_ARTIFACTS_PATH_SUFFIX, DEFAULT_NAS_PATH_SUFFIX
};