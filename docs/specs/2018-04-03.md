...
key | [`key configuration object`](#OSS-Key-configuration-object) | **Required** Indicates the key supported by the filter.
...
filter   | [`filter configuration object`](#CDN-Filter-configuration-object) | **Required** Filters (Need to have at least one filter).
