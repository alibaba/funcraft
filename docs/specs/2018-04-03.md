...
The default role will be generated when the policy is specified or when the service is configured with attributes such as vpc and nas that explicitly require specific permissions.

The reason for this design is:

1. Because the generation role user needs to assign a large privilege to the sub-user, in unnecessary scenarios, we try not to generate the default role as much as possible to avoid the user's sub-user privilege is insufficient.
2. Generally, the use scenario of this role is used to invoke other cloud services. Even if default role is generated, it is meaningless not to specify policies, because there will be no corresponding permissions.
3. ram has a limitation on the number of roles.