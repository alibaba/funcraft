...
PublishVersion | `boolean` | 如果PublishVersion为true，会为此服务发布新版本。
...
属性名称 | 类型 | 描述
---|:---:|---
ServiceName | `string` | **必填。** 别名指向的服务名称.
VersionID | `string` | **必填。** 别名指向的主版本。
Description | `string` | 别名描述。
AdditionalVersionWeight | [灰度版本](#灰度版本对象) | 别名指向的灰度版本以及权重.
