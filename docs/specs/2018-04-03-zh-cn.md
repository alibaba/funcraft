...
属性名称 | 类型 | 描述
---|:---:|---
Protocol | `string` | 自定义域名支持的协议类型。可选值为：`HTTP` 或 `HTTP,HTTPS`
RouteConfig | [PathConfig](#pathConfig) 数组 | 路由表，配置路径和函数的映射关系。
CertConfig | [CertConfig](#certConfig) 对象 | https 证书信息
...
```
  CertConfig:
    CertName: 'CertName'
    PrivateKey: '-----BEGIN RSA PRIVATE KEY-----xxxxxxxxxx-----END RSA PRIVATE KEY-----'
    Certificate: '-----BEGIN CERTIFICATE-----xxxxxxxxxxxxx-----END CERTIFICATE-----'
