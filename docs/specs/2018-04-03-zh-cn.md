...
key | [key 对象](#key-配置对象) | **必填。** 过滤器支持过滤的对象键
...
filter   | `object` | **必填。** 过滤器（至少需要一个过滤器），为 CDN 过滤参数，满足过滤条件的才可以触发函数，包含一个配置属性 domain
domain | `array` | **必填。** 过滤参数值的集合
...
key 配置对象包含的属性包括： `prefix` 和 `suffix` 属性。它们所代表的含义分别为：匹配前缀和匹配后缀。
