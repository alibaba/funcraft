...
本文件中的 “MUST”，“MUST NOT”，“REQUIRED”，“SHALL”，“SHALL NOT”，“SHOULD”，“SHOULD NOT”，“RECOMMENDED”，“MAY”，和 “OPTIONAL” 按照 [RFC 2119](http://www.ietf.org/rfc/rfc2119.txt) 中的描述进行解释。
...
Serverless 应用是由事件触发功能组成的应用。一个典型的 serverless 应用由一个或多个由诸如向 [阿里云 OSS](https://www.alibabacloud.com/product/oss) 上传对象，在 [阿里云 OTS](https://www.alibabacloud.com/product/table-store) 上执行数据操作以及 API 操作等事件触发的阿里云函数计算组成。这些函数可以独立使用。也可以利用其它资源，例如阿里云 OTS 的表和 OSS 的 buckets。最基本的 serverless 应用可以只有一个函数。
...
阿里云 Fun 用来描述一个 serverless 应用的文件有 [JSON](http://www.json.org/) 和 [YAML](http://yaml.org/spec/1.1/) 格式化的文本文件。这些文件与 [ROS 模板](https://www.alibabacloud.com/help/doc-detail/28852.htm) 兼容。

阿里云 Fun 引入了几种新的资源和属性类型，可以嵌入到模板的 [Resources](https://www.alibabacloud.com/help/doc-detail/28863.htm) 部分。模板可能包含所有其他模板部分，并使用 [ROS 内置函数](https://www.alibabacloud.com/help/doc-detail/28865.htm) 来访问仅在运行时可用的属性。
...
创建一个 FC 服务。服务是一个函数组，绑定函数公共属性，如 [RAM](https://www.alibabacloud.com/product/ram) 执行角色.

##### 属性

属性名称 | 类型 | 描述
---|:---:|---
ServiceName | `string` | 函数的名称。如果你不想指定该名称，将为您生成一个全局唯一的名称。 [更多信息](https://www.alibabacloud.com/help/doc-detail/52077.htm)
Role | `string` | 使用一个 RAM 角色的 ARN 为函数指定执行角色。 如果忽略，将为函数创建一个默认的角色。
Policies | `string` <span>&#124;</span> `string` 列表 <span>&#124;</span> [RAM policy 文档对象](https://www.alibabacloud.com/help/doc-detail/28663.htm) <span>&#124;</span> [RAM policy 文档对象](https://www.alibabacloud.com/help/doc-detail/28663.htm) 列表 | 这个函数需要的阿里云管理的 RAM policies 或 RAM policy 文档的名称，应该附加到该函数的默认角色上。如果设置了 Role 属性，则该属性没有意义。
...
FunctionName | `string` | 函数的名称。如果你不想指定该名称，将为您生成一个全局唯一的名称。 [更多信息](https://www.alibabacloud.com/help/doc-detail/52077.htm)
...
当此资源的逻辑 ID 提供给 [Ref](https://www.alibabacloud.com/help/doc-detail/28865.htm?spm=a3c0i.o28852en.b99.21.13b17874UM6SfP#Ref) 内置函数，它将返回底层 FC 函数的资源名称。

###### Fn::GetAtt

当资源的逻辑ID被提供给 [Fn::GetAtt](https://www.alibabacloud.com/help/doc-detail/28865.htm?spm=a3c0i.o28852en.b99.21.13b17874UM6SfP#Fn::GetAtt) 内置函数，它会为此类型的指定属性返回一个值。本节列出了可用的属性。
...
当此资源的逻辑 ID 提供给 [Ref 内置函数](ttps://www.alibabacloud.com/help/doc-detail/28865.htm?spm=a3c0i.o28852en.b99.21.13b17874UM6SfP# Ref)，它会返回底层 API 网关 RestApi 的资源名称。
...
当此资源的逻辑ID提供给 [Ref](https://www.alibabacloud.com/help/doc-detail/28865.htm?spm=a3c0i.o28852en.b99.21.13b17874UM6SfP#Ref) 内置函数中，它返回底层 TableStore 的资源名称。
...
Events | `string` <span>&#124;</span> `string` 的列表 | **必填。** 有关 OSS 事件类型有效值，请参见 [阿里云 OSS 支持的 事件类型](https://www.alibabacloud.com/help/doc-detail/53102.htm) .
Filter | [OSS 对象过滤器](https://www.alibabacloud.com/help/doc-detail/53102.htm) | 用于过滤事件的规则。
...
RestApiId | `string` | 必须包含具有给定路径和方法的操作的 RestApi 资源的标识符。通常，这设置为 [参考](https://www.alibabacloud.com/help/doc-detail/28865.htm?spm=a3c0i.o28852en.b99.21.13b17874UM6SfP#Ref) 在此模板中定义的一个 `Aliyun :: Serverless :: Api` 资源。如果未定义，则使用生成的 Swagger 文档创建默认的 `Aliyun :: Serverless :: Api` 资源，该文档包含由此模板中定义的 `Api` 事件定义的所有路径和方法的联合，它们没有指定 RestApiId。
...
属性名称 | 类型 | 描述
---|:---:|---
ReadCapacityUnits | `number` | 在 Amazon DynamoDB 平衡负载之前，为指定的表设置每秒所需项目的最少一致读取次数（最大大小为 4KB）。
WriteCapacityUnits | `number` | 在 Amazon DynamoDB 平衡负载之前，为指定表设置每秒所需项目的最小一致写入次数（最大大小为 4KB）。
